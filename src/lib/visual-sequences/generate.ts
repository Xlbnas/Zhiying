...
import {buildVisualSequencesUserPrompt, VISUAL_SEQUENCES_SYSTEM_PROMPT} from './prompt';
import {buildSequencePlannerInput} from './projection';
import {
  VISUAL_SEQUENCES_PROMPT_VERSION,
  visualSequencesProposalSchema,
  type VisualSequenceV1,
} from './schema';
import {validateVisualSequences, SEQUENCE_NON_BLOCKING_CODES, type SequenceValidationIssue} from './validate';

/** usage 记录的 stage 标签（同时作 generation_runs/dispatch 的 stage）。 */
export const SEQUENCES_USAGE_STAGE = 'm7_visual_sequences';

const MAX_REPAIRS = 2;
const MAX_OUTPUT_IN_REPAIR = 4000;
const MAX_ISSUES_IN_REPAIR = 2000;

export interface VisualSequencesGenerationResult {
...
function formatIssues(issues: SequenceValidationIssue[]): string {
  return issues
    .slice(0, 10)
...
export async function generateVisualSequences(options: {
  db: Db;
  ...
  ownerToken: string;
  signal?: AbortSignal;
}): Promise<VisualSequencesGenerationResult> {
  const {db, provider, beats, plan, intentPlan} = options;
  const plannerInput = buildSequencePlannerInput(beats, plan, intentPlan);
  const originalUser = buildVisualSequencesUserPrompt(plannerInput);
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    if (options.signal?.aborted) {
      throw new LLMError('CANCELLED', `visual sequences 已被取消（第 ${attempt} 次尝试前）`);
    }
    ...
    const request: LLMRequest = {
      ...
      system: VISUAL_SEQUENCES_SYSTEM_PROMPT,
      user,
      outputMode: 'json',
      thinking: 'disabled',
      maxTokens: 8192,
      signal: options.signal,
      meta: {stage: SEQUENCES_USAGE_STAGE},
    };
    ...
    if (response.text.trim().length === 0) {
      ...
      throw new LLMError('EMPTY_RESPONSE', `visual sequences 第 ${attempt} 次请求仍返回空内容`);
    }
    if (response.finishReason === 'length') {
      ...
      throw new LLMError(
        'OUTPUT_TRUNCATED',
        'visual sequences 输出被 maxTokens=8192 截断（finishReason=length）',
      );
    }

    // parse → zod（LLM 输出契约：只允许 sequences body）
    ...
    try {
      ...
      if (attempt === maxAttempts) {
        throw new LLMError(
          'VALIDATION_FAILED',
          `visual sequences 经 ${MAX_REPAIRS} 次 repair 仍失败。${lastFailure}`,
        );
      }
    ...
    const safe = visualSequencesProposalSchema.safeParse(parsed);
    if (!safe.success) {
      ...
      if (attempt === maxAttempts) {
        throw new LLMError(
          'VALIDATION_FAILED',
          `visual sequences 经 ${MAX_REPAIRS} 次 repair 仍失败。\n${clipText(lastFailure, MAX_ISSUES_IN_REPAIR)}`,
        );
      }
      user = buildRepairUser(originalUser, response.text, lastFailure);
      continue;
    ...
    // deterministic semantic validation（覆盖/连续/chapter/intent 引用/continuation）。
    // 非阻断 issue（SEQUENCE_NEEDS_REVIEW：VISUAL_UNRESOLVED 允许保留）不触发 repair，
    // 由 classify 映射 needs_review——绝不把 unresolved 自动改写为 MG。
    const semanticIssues = validateVisualSequences(beats.beats, intentPlan.intents, safe.data.sequences);
    const blockingIssues = semanticIssues.filter((issue) => !SEQUENCE_NON_BLOCKING_CODES.has(issue.code));
  ...
  throw new LLMError('VALIDATION_FAILED', `visual sequences 未知失败：${lastFailure}`);
}
