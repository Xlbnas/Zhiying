...
| **TTS-C.1B.3** | adapter reload 接入（§C 步骤 4）+ activation acknowledgment（步骤 5）+ atomic activation（步骤 6）+ recovery/reconciler（§D）。**implementation in progress on work branch `work/tts-c1b3-activation-recovery`（initial Independent Review = FAIL `570c448…` → R1 blocker repair implemented：fenced indeterminate / uncertainty 闭环 / stable rollback / legacy rollback / per-HTTP renew / path containment / adapter error contract（R1 blocker-specific Review = FAIL）→ R2 blocker repair implemented：active 文件 mutation 紧邻 fenced renew / indeterminate→failed atomic legacy rollback / pre-promotion（building/candidate_persisted）legacy rollback / exact final parent dir fsync（R2 blocker-specific Review = FAIL）→ R3 API-safety closure implemented：post-promotion failed/cancelled 唯一公开入口 = async safe orchestrator（restoreStableAndConfirm confirmed 后才 fenced terminal；rollback_pending/unknown 不 terminal 不释放 single-flight）；低层同步 terminal helper 收为模块私有；pre-promotion helper 保持公开（R3 final blocker-specific Review = FAIL：`registry-publisher.failPublication` 仍允许 file_durable direct terminal）→ R4 exported-API closure implemented：公开 `failPublication` 已移除；不存在仅凭 db+publicationId+owner+attempt 即可把 file_durable/activation_pending 推进 terminal 的公开函数；legacy pre-promotion 只能走 fail/cancelPrePromotionPublicationAndRollbackLegacy（atomic legacy rollback）→ **R4 Final Blocker-Specific Review = PASS；ALL IDENTIFIED 1B.3 BLOCKERS CLOSED；READY FOR INTEGRATION TO m7；not yet deployed；not yet frozen；测试 259 PASS；combined gate suite 56）** | 依赖 1B.2 |
...
→ Topology-Specific Review = FAIL → Deployment Topology Closure R1 implemented（R1 三个原始
blocker 关闭）→ Topology R1 Review = FAIL（materialization rollback 缺反向 relocation）→
Deployment Topology R2 rollback procedure closure implemented（docs-only）→ **Deployment
Topology R2 Final Topology-Specific Review = PASS；ALL IDENTIFIED DEPLOYMENT TOPOLOGY
BLOCKERS CLOSED；READY FOR TOPOLOGY INTEGRATION TO m7；reviewed topology SHA =
`6f2f1ede5e72974ed36efd5a5cd2f8cb58f21d47`；Integrated Topology exact-SHA Review = PASS
→ **PRODUCTION DEPLOYMENT = PASS（2026-08-07；deployed production runtime
`cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`）→ **Deployment Evidence Review = PASS →
TTS-C.1B.3 = FROZEN（frozen production runtime = `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；
deployment evidence docs = `c4dc544642cf7e1ce54648430d8eee050c5651a1`）**。production
runtime = `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；production POST remains disabled。
...
**A. active registry：single-file bind → directory mount**。单文件 bind 的目标 inode 不能安全
作为 publisher `temp→fsync→rename` 的 replacement（可能 EBUSY），因此最终拓扑为「一个宿主
...
adapter URL                      = 既有 INDEXTTS2_BASE_URL=http://127.0.0.1:9880
                                   （worker 1B.3 wiring：ADAPTER_BASE_URL ?? INDEXTTS2_BASE_URL）
```
...
```text
DB destination_voice_root_relative_path = <profileId>/<revisionId>/reference.wav
...
2. 记录 SHA/size/mtime/inode（1dab4a31… / 322 / 2026-07-25… / 71982）
3. 创建目标目录：/vol1/1000/docker/zhiying/voice-registry（mkdir；lstat 非 symlink）
4. 复制当前 bytes → voice-registry/voice-registry.json（零内容变化）
5. fsync 文件（fsync + close）
6. fsync 目录
7. SHA 相等（sha256sum 比对 1dab4a31…）
8. lstat/realpath 无 symlink、普通文件
9. 仅全部通过后，才修改 compose/.env（ZHIYING_HOST_VOICE_REGISTRY →
