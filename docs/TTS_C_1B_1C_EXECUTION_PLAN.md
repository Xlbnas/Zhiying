...
> disabled；**TTS-C.1B.2 deployed to production**（审计链：initial Independent Review = FAIL
...
> deployment PASS（§15.17；pending Deployment Evidence Review）**；production runtime =
> `6874f51c717ebab1c282ee29e9301f27627deaf7`；测试 138 PASS；combined gate suite 55）；
...
> Integrated exact-SHA Review = PASS；已随 1B.2 部署 production）**：① P0-A
...
| **TTS-C.1B.2** | legacy import（§F）+ publisher candidate creation（§C 步骤 1-3：T1 claim、candidate 构建、T2 persist 到 file_durable）+ publication 状态推进；**不 reload adapter**。**deployed to production（审计链：initial Independent Review FAIL `08be813…` → R1 blocker-specific Review PASS `bcfd29b…` → Integrated exact-SHA Review PASS；测试 138 PASS；combined gate suite 55；pending Deployment Evidence Review）** | 依赖 1B.1 contract（registry JSON schema 与 ack 字段冻结） |
