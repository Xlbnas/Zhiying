...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-03T17:10Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R7 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1A not started**） |
...
### 11.4 TTS-C.0.R7 docs-only SQLite contract 验证（临时，不入仓库；runtime 阶段纳入 gate）

临时目录 sqlite3 3.45.1（2024-01-30 16:01:20）：**直接从最终设计文档 §2 提取全部可执行 SQL**（10 表：含第 10 表
`voice_registry_publications`）重建临时 DB → `PRAGMA foreign_key_check`（空）→ `PRAGMA integrity_check`（ok）→
happy path 全链（synthesis 新生成 / reuse fan-in / materialization 全链 / **publication journal 全流程**
（building→candidate_persisted→file_durable→activation_pending→active + projection published_usable + legacy
mapped_active）/ legacy requeue 正向控制）→ **crash-retry 闭环**（failed publication A → 新 attempt B 全流程成功 →
A/B evidence 均保留、active 无泄漏、A evidence 不可改）→ **139 项 mutation 验证全部 PASS，FAIL=0**：
