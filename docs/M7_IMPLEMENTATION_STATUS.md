...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-06T08:30Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0 = FROZEN（独立 Review PASS）**；**TTS-C.1A = FROZEN（R7 Final Proportional Review PASS + Deployment Evidence Review PASS；production runtime 历史 = `37eaac6c8c8969239cab00848f6291454615a912`；POST remains disabled）**；**TTS-C.1B.1 = FROZEN**（Independent Review PASS + R1 blocker-specific Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS + Deployment Evidence Review PASS，deployment evidence `docs/evidence/tts-c-r17/deployment-c1b1-c1c1.md`）；**TTS-C.1C.1 = FROZEN**（Independent Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS + Deployment Evidence Review PASS）；**Deployment Evidence Review = PASS**；**TTS-C.1B.2 merged to m7**（审计链：initial Independent Review FAIL `08be813…` → R1 blocker-specific Review PASS `bcfd29b…` → R1 全部 blocker 关闭 → **merged to m7；pending Integrated exact-SHA Review；not deployed**）；production runtime = `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；POST remains disabled；TTS-C.1B.3 not started；TTS-C.1C.2 not started；TTS-C.2 not authorized；combined quality gate suite count = 55） |
...
| productionRuntimeSHA | `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`（TTS-C.1B.1 + 1C.1 部署后容器镜像实际代码 SHA；composition 与上一 runtime `37eaac6…` 的差异仅为本次合入的 1B.1 adapter contract + R1 reference 文件前置验证 + 1C.1 capability snapshot + pure compiler + tests + gate + docs；§2 SQL SHA `c88f64ac…` 不变 = 零 schema 迁移；production runtime code 内容以 `git rev-parse origin/m7` 为准；TTS-C.0 freeze 基线与 CI 证据见 §11.4） |
| productionHostCheckoutSHA | `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`（宿主机 checkout = `ZHIYING_RELEASE_TAG`，与本轮 deployed 镜像代码 SHA 一致；可因 docs/ops commit 高于 runtime） |
| 当前分支 | `m7` |
...
- **TTS-C.1B.2 merged to m7（审计链：initial Independent Review = FAIL `08be81316d07a382c92a4a11855b3b5f06055b87` → TTS-C.1B.2.R1 exact SHA = `bcfd29b345e59c8003fe38c8ec6d80f4df83e19e` → R1 blocker-specific Review = PASS → all identified blockers closed → merged to m7 → **pending Integrated exact-SHA Review**；not deployed；production 不变）**：legacy import + publisher candidate creation（T1 claim / candidate 确定性构建 / T2 file_durable）——见实施计划 `docs/TTS_C_1B_1C_EXECUTION_PLAN.md` §C 步骤 1-3 / §F / §J。真实代码入口：
