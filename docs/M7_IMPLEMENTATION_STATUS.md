...
| productionRuntimeSHA | `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`（TTS-C.1B.3 部署后容器镜像实际代码 SHA = deployed exact SHA；§2 SQL SHA `c88f64ac…` 不变 = 零 schema 迁移；**production runtime authority = deployed exact SHA + deployed image/container evidence；origin/m7 may advance independently and is not production runtime authority**；TTS-C.0 freeze 基线与 CI 证据见 §11.4） |
| productionHostCheckoutSHA | `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`（宿主机 checkout = `ZHIYING_RELEASE_TAG`，与本轮 deployed 镜像代码 SHA 一致；可因 docs/ops commit 高于 runtime） |
| 当前分支 | `m7` |
...
### TTS-C.2 Phase 1 R2 final exact-reread closure（2026-08-07；work branch `work/tts-c2-phase1`；PENDING R2 FINAL BLOCKER-SPECIFIC REVIEW）
