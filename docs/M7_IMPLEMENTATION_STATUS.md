...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T14:30Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0 = FROZEN（独立 Review PASS）**；**TTS-C.1A deployed（pending independent Review PASS）**；TTS-C.1B / 1C / C.2 not started） |
...
| productionRuntimeSHA | `8ab1b235fe65d18a0362594a36ce48b9ef314e9b`（TTS-C.1A 部署后容器镜像实际代码 SHA；TTS-C.0 freeze 基线与 CI 证据见 §11.4） |
| productionHostCheckoutSHA | `8ab1b235fe65d18a0362594a36ce48b9ef314e9b`（宿主机 checkout；可因 docs/ops commit 高于 runtime） |
| 当前分支 | `m7` |
