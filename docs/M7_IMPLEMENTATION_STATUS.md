...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T05:34Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R9 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1A not started**） |
...
- 下一步：TTS-C.1A（voice materialization durable requests/files/DB projection，schema 按 R9 contract）→ TTS-C.1B（global registry publisher + adapter hot reload，subject_mode 双路径就位）→ TTS-C.1C（capability snapshot/compiler）→ TTS-C.2（request envelope + durable job + attempt journal + immutable sentence audio artifact + fingerprints + `tts_job_execution_transitions` atomic coupling）→ TTS-C.3（preview/override/variant/A-B/selection）→ TTS-C.4（selection manifest + immutable master audio + ffprobe）→ TTS-C.5（subtitle timing v2/reconciliation/stale graph/review UI）→ timing-reconciliation@2.0 → storyboard → animatic → final render。TTS-C.1A 未开始，须待本 R9 独立 Review PASS。
...
### 11.4 TTS-C.0.R9 docs-only SQLite contract 验证（临时，不入仓库；runtime 阶段纳入 gate）

临时目录 sqlite3 3.45.1（2024-01-30 16:01:20）+ 当前 Python sqlite3：**直接从最终设计文档 §2 提取全部
可执行 SQL**（13 表：含第 11 表 `voice_registry_publication_activations`、第 12 表
`tts_claim_generation_dispatches`、第 13 表 `tts_job_execution_transitions`）重建临时 DB →
`PRAGMA foreign_key_check`（空）→ `PRAGMA integrity_check`（ok）→ happy path 51 项（synthesis 全链含
**单条 atomic dispatch**（CJ-10）/ reuse fan-in / materialization 全链 / **PA-02 materialization 与 PA-03
legacy_cutover_publish 单条 atomic activation command** / **LC-01 legacy_cutover_publish 路径** /
**LC-02 legacy_cutover_existing 路径**（projection 零 UPDATE）/ **JS-03 worker-claim execution command
同步推 claim/job** / **JS-04 execution command 终态 result artifact 两侧填齐** /
**IE-05 activation_pending→indeterminate→active resolve 闭环** /
**VI-05 voice_revision compat happy path** / registry_rebuild / **crash-retry 闭环**（failed publication A →
新 attempt B 成功，A/B evidence 均保留、active 无泄漏）/ **legacy failed→rollback→新 publication→成功闭环**
（LR-01/03/04）/ **PE-04 activation_indeterminate resolve active** / legacy requeue 正向控制）→
**360 项断言全部按预期，FAIL=0**：

- **R9 新增矩阵 66 项**：
  - **TF（database-time fencing）6 项**：TF-01/02 publication lease 过期回填 `activated_at`/`created_at`/
    `observed_at` → `fencing mismatch` ABORT、TF-03 validation lease 过期回填 dispatch.created_at ABORT、
    TF-04 lease 未过期 + exact token/attempt PASS、TF-05 外部 I/O 在到期前完成但 commit 时 DB 当前时间已越期
    ABORT、TF-06 takeover 后旧 owner 即使回填时间也无法提交；
  - **IE（indeterminate evidence）8 项**：IE-01/02/03/04 进入 indeterminate 同步写 candidate/manifest/file/
    activation 证据 → `indeterminate entry evidence seal` ABORT、IE-05 activation_pending→indeterminate
    原 evidence 不变 + resolve active（owner_token=NULL + resolution_evidence + resolution_evidence_hash +
    attempt 精确匹配）→ PASS、IE-06 resolve attempt mismatch ABORT、IE-07 resolve resolution_evidence NULL
    ABORT、IE-08 exact attempt + exact evidence PASS；
  - **LC（legacy cutover reachability）7 项**：LC-01/02 happy path、LC-03 materialization_publish subject
    被 legacy 引用 → `materialization_publish blocked by legacy mapping` ABORT、LC-04 两个 legacy key
    同 projection → `uq_lve_mapped_materialization` UNIQUE ABORT、LC-05 一对一模式下第二个 mapping
    ABORT、LC-06 cutover_existing 在一对一模式下命中 PASS、LC-07 legacy_cutover_existing activation 不得
    改写已发布 projection evidence PASS；
  - **JS（claim/job state coupling）6 项**：JS-01 job queued→running 单独 UPDATE → `tts_jobs state transition
    requires execution command` ABORT、JS-02 claim generation_pending→running 单独 UPDATE → ABORT、
    JS-03/04 worker-claim execution command 同时更新 job+claim PASS、JS-05/06 不一致 ABORT；
  - **VI（voice identity）2 项**：VI-05 TTS-C job 修改 voice_profile_revision → `immutable field` ABORT、
    VI-06 TTS-C 行 voice_profile_revision_id=A 但 voice_profile_revision=B → `voice_profile_revision
    compat mismatch` ABORT；
  - **ET（evidence timestamp freeze）3 项**、**SM（subject_mode 联合判定）5 项**、**GN（generation
    uniqueness）2 项**（完整断言文本见设计文档 §10.8）；
...
- **R7 回归 47 项（适配 R9 schema）**：RP-02/02b/03/03b/04/05/09/10/10b/11/12、CJ-01…07、SL-01…08b、
  VI-01…04b（VI-02b dispatch 旧 voice_profile_revision 必填 NOT NULL 由 `trg_tts_jobs_revision_compat`
  双 trigger 强化）；
...
每个反例的预期错误文本或 changes=0 已逐一断言（同表多 trigger 按创建逆序触发；BEFORE INSERT trigger 先于 FK/CHECK
enforcement——冻结消息以实证为准）。临时 SQL/DB 未入仓库。R9 全量 R5/R6/R7/R8 回归通过。
