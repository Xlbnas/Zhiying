...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-03T11:15Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R1 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1 not started**） |
...
- **TTS-C.0.R1 architecture closure completed（pending independent Review PASS；runtime implementation not started；TTS-C.1 not started；只读审计，零代码/零部署）**：关闭独立 Review FAIL 发现——① **mutable job 与 immutable artifact 分离**（废弃「tts_jobs 行即 sentence audio artifact」：`tts_audio_requests` envelope / `tts_jobs` mutable execution / `tts_generation_attempts` append-only journal / `sentence_audio_artifacts` immutable result（trigger ABORT + exact reader），`tts_jobs.output_path/duration_ms/audio_sha256/result_json` 降级 legacy 兼容）；② **requestId/single-flight/attempt/indeterminate**（DB 级约束区分 request idempotency / synthesis identity / generation variation；indeterminate 禁自动重调 provider）；③ **manifest/master 非循环身份**（`narration_audio_selection_manifest@2.0` 不含 master 信息 + `narration_master_audio@1.0` 引用 manifest，masterInputFingerprint 无自引用）；④ **materialization 真实拓扑**（Worker 唯一 writer、Web 无 voice 挂载、`/voice-config` 目录挂载、global registry publisher 从 DB 全量重建 + 锁、adapter hot reload last-known-good、archive 后合法历史 Assignment 的 revision 仍可 materialize）；⑤ **fingerprint 三分离**（exactSourceFingerprint / synthesisPayloadFingerprint / finalTtsInputFingerprint；跨 source 复用需 immutable acoustic_equivalence_attestation）；⑥ **capability neutral matrix**（neutral 默认值 → supported no-op 而非 unsupported）；⑦ **OSS exact-SHA 矩阵**（5 项目 exact commit SHA + license；calesthio/OpenMontage AGPL-3.0 = Reject，只概念参考）；⑧ **milestone 拆分 1A/1B/1C + 2-5**。三份 TTS-C 文档全部修订；TTS-B/A 冻结语义未动。
