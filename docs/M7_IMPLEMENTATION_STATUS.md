...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-07T16:21Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0 = FROZEN（独立 Review PASS）**；**TTS-C.1A = FROZEN（R7 Final Proportional Review PASS + Deployment Evidence Review PASS；production runtime 历史 = `37eaac6c8c8969239cab00848f6291454615a912`；POST remains disabled）**；**TTS-C.1B.1 = FROZEN**（Independent Review PASS + R1 blocker-specific Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS + Deployment Evidence Review PASS，deployment evidence `docs/evidence/tts-c-r17/deployment-c1b1-c1c1.md`）；**TTS-C.1C.1 = FROZEN**（Independent Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS + Deployment Evidence Review PASS）；**Deployment Evidence Review = PASS**；**TTS-C.1B.2 = FROZEN**（审计链：initial Independent Review FAIL `08be813…` → R1 blocker-specific Review PASS `bcfd29b…` → Integrated exact-SHA Review PASS → production deployment PASS → **Deployment Evidence Review = PASS**；frozen production runtime = `6874f51c717ebab1c282ee29e9301f27627deaf7`；deployment evidence docs SHA = `93cee1d5afec358cd281e97578e9a691dcbd3a7f`）；**TTS-C.1B.3 historical implementation/review chain（work branch `work/tts-c1b3-activation-recovery`；initial Independent Review = FAIL `570c448…` → TTS-C.1B.3.R1 blocker repair implemented（R1 blocker-specific Review = FAIL）→ TTS-C.1B.3.R2 blocker repair implemented（R2 blocker-specific Review = FAIL）→ TTS-C.1B.3.R3 API-safety closure implemented（R3 final blocker-specific Review = FAIL：`registry-publisher.failPublication` 仍允许 file_durable direct terminal）→ TTS-C.1B.3.R4 exported-API closure implemented（公开 `failPublication` 已移除——不再存在仅凭 db+publicationId+owner+attempt 即可把 file_durable/activation_pending 推进 terminal 的公开函数；legacy pre-promotion 只能走 fail/cancelPrePromotionPublicationAndRollbackLegacy）→ TTS-C.1B.3.R4 Final Blocker-Specific Review = PASS；ALL IDENTIFIED 1B.3 BLOCKERS CLOSED；TTS-C.1B.3 READY FOR INTEGRATION TO m7；reviewed implementation SHA = `aad06c0c89ffc795c366df6f98c5eba54cffa4f1`；not yet deployed；not yet frozen）→ TTS-C.1B.3 Integrated exact-SHA Review = PASS（integrated SHA `8203ec99f8434709fca30d6bdd75f3adf63f20c1`）→ TTS-C.1B.3 Deployment Topology Closure implemented（R0）→ Topology-Specific Review = FAIL（R0：`/voices/tts-a` child bind 致 Worker/Adapter host-source identity 可漂移 + worker registry target 与 `/app/data` 重叠）→ TTS-C.1B.3 Deployment Topology Closure R1 implemented（single unified voice root、无 /voices child bind、materializations 物理位于 voice-root/tts-a、registry 独立 /registry target、empty-publication no-op proof；R1 三个原始 blocker 关闭）→ Topology R1 Review = FAIL（唯一剩余 P1：materialization rollback 未包含反向 relocation）→ TTS-C.1B.3 Deployment Topology R2 rollback procedure closure implemented（docs-only：reverse relocation / collision handling / inventory-SHA equality / old runtime visibility / rollback boundary / pre-deploy backup）→ Deployment Topology R2 Final Topology-Specific Review = PASS；ALL IDENTIFIED DEPLOYMENT TOPOLOGY BLOCKERS CLOSED；READY FOR TOPOLOGY INTEGRATION TO m7；reviewed topology SHA = `6f2f1ede5e72974ed36efd5a5cd2f8cb58f21d47`；Integrated Topology exact-SHA Review = PASS → TTS-C.1B.3 PRODUCTION DEPLOYMENT = PASS（2026-08-07；deployed production runtime = `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；previous runtime = `6874f51c717ebab1c282ee29e9301f27627deaf7`）→ **Deployment Evidence Review = PASS → TTS-C.1B.3 = FROZEN（frozen production runtime = `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；deployment evidence docs commit = `c4dc544642cf7e1ce54648430d8eee050c5651a1`）**；production runtime = `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；POST remains disabled；TTS-C.1C.2 not started；TTS-C.2 not authorized；**TTS-C.2 Phase 1 + TTS-C.1C.2 IMPLEMENTED on work branch `work/tts-c2-phase1`（frozen §2 C.2 migration：6 表 + tts_jobs additive；request envelope / claim fan-in / validation reuse / zero-subscriber / atomic dispatch / execution commands / attempts / immutable artifact / capability provenance；R1 blocker repair implemented；R2 FINAL EXACT-REREAD CLOSURE IMPLEMENTED → **R2 Final Blocker-Specific Review = PASS（P0=0，P1=0；ALL IDENTIFIED PHASE-1 BLOCKERS CLOSED）→ FAST-FORWARD INTEGRATED TO m7 → PRODUCTION DEPLOYMENT = PASS（reviewed/integrated/deployed runtime `8e92ba58a28900509e542394858200b1731d7afd`；deployment evidence `docs/evidence/tts-c2/phase1-deployment.md`；PENDING DEPLOYMENT EVIDENCE REVIEW；NOT FROZEN）**）**；combined quality gate suite count = 57） |
...
| productionRuntimeSHA | `8e92ba58a28900509e542394858200b1731d7afd`（TTS-C.2 Phase 1 部署后容器镜像实际代码 SHA = deployed exact SHA；C.2 migration 已由新 runtime 首次打开 DB 时应用：6 新表 + tts_jobs 10 additive 列 + 4 index + 65 trigger；legacy 351 行零改动（claim_id 全 NULL，C.2 字段全默认）；**production runtime authority = deployed exact SHA + deployed image/container evidence；origin/m7 may advance independently and is not production runtime authority**；TTS-C.0 freeze 基线与 CI 证据见 §11.4） |
| productionHostCheckoutSHA | `8e92ba58a28900509e542394858200b1731d7afd`（宿主机 checkout = `ZHIYING_RELEASE_TAG`，与本轮 deployed 镜像代码 SHA 一致；可因 docs/ops commit 高于 runtime） |
| 当前分支 | `m7` |
...
- **TTS-C.1A = FROZEN（R7 Final Proportional Review PASS + Deployment Evidence Review PASS）**：final code/runtime = `37eaac6c8c8969239cab00848f6291454615a912`（runtime code 内容来自 `17d40787ce70c025d7daa012c04a76bc69c10a2b`，`37eaac6…` 仅多 complexity-policy docs）；deployment evidence docs = `c29801b3b313a41560e4e0547033c2a409ed244c`（`docs/evidence/tts-c-r17/deployment.md`，§15.15）。冻结语义：projection 止于 `file_ready_unpublished`；不发布 registry、不激活 adapter、不 cutover、不合成 TTS；production POST remains disabled（`TTS_C1A_MATERIALIZATION_POST_ENABLED` 未设置）；publication/activation/legacy 三表保持 0 行写入。R7 为 TTS-C.1A 防御复杂度上限（proportional-risk rubric；不启动 R8；见 `docs/TTS_C_1A_MATERIALIZATION_IMPLEMENTATION.md` §15-16）。TTS-C.1B.1 / TTS-C.1C.1 已 deployed（详见 §15.16）；TTS-C.1B.2 = FROZEN（§15.17）；TTS-C.1B.3 = FROZEN；TTS-C.1C.2 = Phase 1 随 C.2 部署（PENDING DEPLOYMENT EVIDENCE REVIEW；NOT FROZEN）；TTS-C.2 Phase 1 deployed（§15.18；PENDING DEPLOYMENT EVIDENCE REVIEW；NOT FROZEN）。
- **TTS-C.1B.1 = FROZEN（Independent Review PASS + R1 blocker-specific Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS；deployment evidence 见 §15.16）**：final code/runtime = `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；具体落地：
  - **adapter registry reload contract**（`services/indextts2-api-adapter/server.py`，app version 1.2.0）：registry schema `"1.0"` legacy 与 `"1.1"` publisher 双支持向后兼容（1.0 内部 `generation/publisherSchemaVersion=null`；1.1 要求 `registryGeneration` positive integer + `publisherSchemaVersion` 精确 `tts-c-registry-publisher@1`，voices 沿用既有严格校验；未知 schemaVersion → `VOICE_REGISTRY_UNSUPPORTED_SCHEMA`，1.1 字段缺失/非法 → `VOICE_REGISTRY_INVALID`）；adapter 绝不自动改写 registry 文件。
  - **runtime state**：`RegistryState` 不可变整体替换（`status / voices / loadedRegistrySha256（=当前加载文件原始 bytes 的单一 SHA-256，frozen registry identity）/ loadedRegistryGeneration / publisherSchemaVersion / schemaVersion / lastReloadError / degraded`）；swap = 单引用赋值 + `threading.Lock`，reader 持一次快照引用。无 capability / WeakMap / token / 形式化状态机。
  - **`POST /reload`**：只读取 `ADAPTER_VOICE_REGISTRY_PATH` 固定配置路径，完整验证 + 一次性原子替换 + 返回新 registry status（200）；失败有 LKG → 保持旧 state + `degraded=true` + `lastReloadError` + 非 2xx `VOICE_REGISTRY_RELOAD_FAILED`；无 LKG → `ready=false` + synthesize 503。
  - **reference 文件验证前置（TTS-C.1B.1.R1 blocker fixed）**：`_load_registry_file()` 在给出任何 OK/ack 前，对每个 voice 的 reference 文件做完整验证（存在 + 普通文件 + 可读 + 实际 SHA-256 == registry `referenceSha256`，统一 `_validate_reference_file()` helper，带 `(mtime_ns, size)` 缓存，成功 reload 后首次 synthesize 不重复读文件）；任一 reference 失败 → 本次加载非 OK（错误码**复用既有 frozen 语义** `REFERENCE_VOICE_MISSING` / `REFERENCE_SHA256_MISMATCH`，不引入新码）；synthesize 对 `REFERENCE_*` 状态透传原码，对 `VOICE_REGISTRY_*` 保持既有 `VOICE_REGISTRY_INVALID` 聚合码。
  - **`GET /registry-status`**（唯一 activation acknowledgment 观察面，始终 200）：`ready / degraded / schemaVersion / loadedRegistrySha256 / loadedRegistryGeneration / publisherSchemaVersion / speakerCount / detail / lastReloadError`。
  - **`GET /health`**：保持 `{ready, provider, model, detail}` 兼容，仅增加 `degraded`；LKG degraded 视为 healthy + `detail=最近 reload 失败码`。
  - **`POST /v1/synthesize`**：请求/响应语义不变。
  - 测试：`scripts/test-tts-c1b1-adapter-registry.ts` 34 PASS（六场景 + R1 五项，mock upstream + 临时目录，独立运行 ×2 无进程/端口泄漏）；combined gate suite 54；adapter venv bootstrap 由 `scripts/run-tts-c1b1-adapter-registry.sh` 管理（gitignored 本地环境，bootstrap 失败 → suite 失败 fail-closed）。
  - 1B.1 **未实现**：publisher、legacy import、DB publication 写入、activation、recovery、production 拓扑变更（registry 单文件 bind → 目录挂载、adapter voice-materializations `:ro` 挂载）；未触及 production active registry；未 reload production adapter。下一阶段（1B.2）须独立部署 gate + 拓扑变更独立 review。
- **TTS-C.1C.1 = FROZEN（Independent Review PASS + Integrated exact-SHA Review PASS + Production deployment PASS）**：final code/runtime = `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；具体落地：
  - **provider capability snapshot v1**（`src/lib/tts-c/provider-capability.ts`）——frozen contract `ProviderCapabilitySnapshotV1`：`provider: 'indextts2'` + `adapterCompatibilityKey: 'indextts2-adapter-registry@1'` + `snapshotVersion: 'indextts2-capability@1'` + `controls` 4 项（`deliveryOverride / pace / energy / emotionSemantic`，全部 `supported: false`，snapshot v1 如实反映 adapter 现状：pydantic SynthesizeRequest 仅 5 字段，非默认 useRandom / emotion 已 422）；controls 对象 `.strict()`，未知键显式拒绝；snapshot 不含 emotionVector / emotionAlpha / emotionReferenceAudio / useRandom——仍为后续产品增强需求（§G.1），未取消。
  - **pure compiler**（`src/lib/tts-c/capability-compiler.ts`，零 DB / 零 IO / 零时钟 / 零环境变量 / 零随机数）：`compilePerformanceToProvider(input, snapshot) → {providerParams, unsupportedFlags, compilerVersion: '1.0', snapshotVersion}`；5 条规则（neutral → no-op、supported non-neutral → 映射、unsupported non-neutral → explicit `unsupportedFlags`，不静默丢弃、schema 外字段 ZodError 拒绝、确定性逐字节一致）；输入面 = TTS-B 冻结字段（`deliveryOverride / pace / energy / emotion{none|semantic×6}`，`.strict()`），`unitId` 等未知字段 schema 拒绝。
  - 测试：`scripts/test-tts-c1c1-capability.ts` 53 PASS；combined gate suite 54。
  - 1C.1 **未实现**：provenance 落库（`synthesis_payload_fingerprint` / `sentence_audio_artifacts.capability_*` 列属 C.2 migration，本轮不在 1C）；不调用真实 synthesis；不消费真实 TTS-B Performance Plan（执行链消费属 C.2）。
- **TTS-C.1B.1 + 1C.1 部署证据（§15.16，`docs/evidence/tts-c-r17/deployment-c1b1-c1c1.md`）**：production runtime `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；adapter `/health` ready + degraded=false、`/registry-status` 返回 legacy 1.0 schema（`schemaVersion:"1.0"`, `loadedRegistrySha256:"1dab4a31…"` == 生产 registry 文件原始 bytes, `loadedRegistryGeneration:null`, `publisherSchemaVersion:null`, `speakerCount:1`, `lastReloadError:null`）；既有 synthesis smoke（最小 fixed text + `default@1` voice）→ 200 audio/wav 59470 B；POST → 503 `MATERIALIZATION_NOT_ENABLED`（POST still disabled）；DB zero-change（per-table CSV byte-compare 全部 PASS，integrity ok + FK empty，tts_jobs 351 行 INSERT dump SHA `780b024f…` 不变，六张 TTS-C 表保持 0 行）；registry 与 reference voice 文件未修改（mtime/size/inode/SHA 不变）；compose / Dockerfile / production env 文件未修改（仅 `ZHIYING_RELEASE_TAG` 由 `37eaac6…` → `01f8536…`）。**Deployment Evidence Review = PASS**（initial FAIL → R1 docs-only 修正 → R1 blocker-specific Review PASS，详见 §15.16 审计记录）。
- **TTS-C.1B.2 = FROZEN（审计链：initial Independent Review = FAIL `08be81316d07a382c92a4a11855b3b5f06055b87` → TTS-C.1B.2.R1 exact SHA = `bcfd29b345e59c8003fe38c8ec6d80f4df83e19e` → R1 blocker-specific Review = PASS → all identified blockers closed → merged to m7 → Integrated exact-SHA Review = PASS → production deployment = PASS → **Deployment Evidence Review = PASS**；deployment evidence 见 §15.17；frozen production runtime `6874f51…`）**：legacy import + publisher candidate creation（T1 claim / candidate 确定性构建 / T2 file_durable）——见实施计划 `docs/TTS_C_1B_1C_EXECUTION_PLAN.md` §C 步骤 1-3 / §F / §J。真实代码入口：
  - `src/lib/tts-c/registry-schema.ts`——registry JSON 1.0/1.1 类型 + 严格校验（与 adapter 1B.1 校验链一致）+ canonical JSON 序列化（键递归排序）+ 单一 SHA-256；
  - `src/lib/tts-c/legacy-import.ts`——`importLegacyRegistry(db, {registryFilePath, voiceRootDir, resolveReferencePath})`：单 BEGIN IMMEDIATE 事务、确定性身份（frozen `UNIQUE(voice_profile_key, voice_revision_key)`）、同内容复用 / 内容漂移 fail-closed 整批回滚、reference 文件存在/普通文件/可读/SHA 前置验证、不修改 registry/reference、只 INSERT unmapped；
  - `src/lib/tts-c/registry-publisher.ts`——`claimPublication`（T1：BEGIN IMMEDIATE + generation=MAX+1 + subject 四态冻结 + owner/lease/attempt=1 + legacy_cutover 同事务 entry→mapping_pending；同 subject 复用 already_in_flight / 异 subject 单飞冲突 PUBLICATION_CONFLICT）、`renewPublicationLease`（T1.5 fenced）、`buildRegistryCandidate`（stable view §7.1 投影 + subject key 换入；1.1 文档 registryGeneration/publisherSchemaVersion；manifest 逐 key emitted source/sourceRowId/referenceSHA/adapterKey；candidate 引用的全部 reference 文件复算 SHA 比对；key 冲突 fail-closed）、`markCandidatePersisted`（Tx A fenced）、`persistCandidateFile`（temp O_EXCL→write→fsync→rename→dir fsync→reread 复算 SHA+JSON/generation 复核；同 generation 同 bytes 复用 / 异 bytes fail-closed；root/final symlink 与越界拒绝）、`markFileDurable`（Tx B fenced）、`publishRegistryCandidate`（T1→T1.5→build→Tx A→file→Tx B 编排，幂等可重跑）、`failPublication`（fenced failed 终态；recovery 用）。candidate 文件约定：`<dataDir>/voice-registries/candidate-<generation>.json`（temp 同目录 stagingTempPath 范式；部署拓扑变更——目录挂载——仍属 1B 部署 gate 独立 review，本轮未实施）。
  - 测试 `scripts/test-tts-c1b2-publisher-candidate.ts` **106 PASS ×2 独立运行**（零进程/fd/temp/端口泄漏）：A legacy import 10 场景（空 DB 首次/重跑幂等/真实双进程并发单权威/一致复用/内容冲突 fail-closed 整批回滚/reference 缺失/SHA 不符/未知 schema/重复 key/文件零修改）；B T1 claim 10 场景（四 subject 类型 claim、非法组合拒绝、同 subject 重放复用、双进程唯一 winner、异 subject 单飞冲突 + 终态后串行、projection 状态 fail-closed；legacy_cutover_existing 合法前置 published_usable 属 1B.3 不可达——断言 frozen gate ABORT）；C candidate 确定性 10 场景（重复构建逐字节一致、manifest 排序、key 冲突、generation MAX+1、1.1 字段完整、publisherSchemaVersion 精确、SHA==bytes、无多余字段、legacy/materialization 合并裁决、输入对象零修改）；D durable file 15 场景（正常 temp→fsync→rename→dir fsync、SHA==DB evidence、fsync/fsyncDir/rename/reread/SHA-mismatch 故障注入、symlink root/final 拒绝、path escape 拒绝、DB 不先于文件、文件 durable 后 DB finalize 失败留 recoverable evidence、重跑复用、同 generation 异 bytes fail-closed、无残留）。combined gate suite **55**。**未执行**：`POST /reload`、activation、cutover、recovery 闭环、production 访问。TTS-C.1B.3 / 1C.2 not started；TTS-C.2 not authorized。
- **TTS-C.1B.2.R1 blocker repair（2026-08-06；R1 blocker-specific Review = PASS；merged to m7；Integrated exact-SHA Review = PASS；已随 1B.2 部署到 production）**：Independent Review FAIL 的 5 项 blocker 最小修复——① **P0-A same-subject loser 分流**：`publishRegistryCandidate` 按 `claim.kind` 分流，loser（already_in_flight）不 renew / 不 build / 不写文件 / 不推进 DB / 不 failPublication；`already_in_flight` 公开返回收窄为无 owner_token 的 `PublicationStatusDto`；file_durable 只做只读 durable verification 后返回 `already_file_durable`；activation_pending/indeterminate 返回范围外状态不启动 1B.3；编排返回 union（completed / already_in_flight / already_file_durable）。未引入 owner handle/capability 框架。② **P0-B existing-final 重建立 durability**：统一 `durabilizeAndVerifyCandidate` acceptance（O_NOFOLLOW 打开 + fstat 普通文件 + fd 读取 + length + SHA + parse 1.1/schemaVersion/registryGeneration/publisherSchemaVersion + final fsync + parent dir fsync），新文件路径与 existing-final 共用；同 SHA 不得直接 return（必须重新 fsync final + parent dir）。③ **P1-A reference containment**：`verifyReferenceFile` 增加 rootDir 参数（root realpath 固定 + 逐级 parent 无 symlink + O_NOFOLLOW + fd 读取 + SHA）；resolver 只做 path translation 无 containment 例外；materialization projection 复用现有 `validateMaterializedFileSnapshot`（hardened validator）。④ **P2 文档一致**：final bytes 语义校验（SHA/length/schemaVersion/registryGeneration/publisherSchemaVersion）在 `persistCandidateFile`/acceptance 内部执行；删除未使用常量。⑤ **exact-HEAD gate**：全部 commit 后 clean HEAD 前台跑 55-suite gate（QUALITY_GATE_SHA == final HEAD）。测试扩展至 **138 PASS ×2 独立运行**（新增 R1-01 双进程完整 orchestrator 唯一 winner / R1-02-03 subscriber 零副作用（lease 不变、无文件、DB 不推进、无 owner token）/ R1-04-05 dir-fsync 失败→重跑重新 fsync 组合 / R1-06 existing final generation 校验 / R1-07 resolver 越界 / R1-08 legacy parent symlink / R1-09 materialization parent symlink / R1-10 final symlink）。未执行：`POST /reload`、activation、cutover、recovery、production 访问。TTS-C.1B.3 / 1C.2 not started；TTS-C.2 not authorized。
  - **证据口径（集成后）**：138 assertions 与 55-suite gate 在 agentvm 本地执行；R1 exact-HEAD gate `QUALITY_GATE_RESULT=PASS`、`QUALITY_GATE_TOTAL_SUITES=55`、`QUALITY_GATE_SHA=bcfd29b345e59c8003fe38c8ec6d80f4df83e19e`（== reviewed branch final HEAD）；该 reviewed SHA **没有新的 GitHub Actions workflow run 或 commit status**，不得写成 GitHub-hosted independent CI evidence；production 证据边界不变（OPS-AUDIT-BRIDGE 不可用）。集成方式：fast-forward（m7 base `3ed4a96…` → reviewed `bcfd29b…`），7 个核心 blob（registry-contract-error / registry-schema / legacy-import / registry-publisher / tts-c1b2-child / test-tts-c1b2-publisher-candidate / run-m7-quality-gate）reviewed SHA 与集成树逐项一致；integration docs commit 后非文档 blob 复验一致。**Integrated exact-SHA Review = PASS → production deployment PASS（§15.17；pending Deployment Evidence Review）**。
...
### 15.16 TTS-C.1B.1 + TTS-C.1C.1 部署证据（01f8536，2026-08-06，docs-only pending Review）

> 本轮为 **exact-SHA、零 schema、零 DB 写入、零 publication、零 registry 改动**的 1B.1 + 1C.1 部署。
> 详情证据：`docs/evidence/tts-c-r17/deployment-c1b1-c1c1.md`（同 commit 内的独立 evidence 文档）。

- **部署链**：deployed SHA `01f8536b4bac1661aa86ad57f90985ec56c8aaa5` = production runtime SHA = host checkout SHA（`ZHIYING_RELEASE_TAG=01f8536…`；本 commit 是上一 runtime `37eaac6…` 后的 12 个 commits：1B.1 adapter contract + R1 reference 验证前置、1C.1 capability snapshot + pure compiler、tests、gate、`scripts/run-m7-quality-gate.sh` +1 suite、`scripts/run-tts-c1b1-adapter-registry.sh` adapter venv bootstrap、docs）；previous runtime `37eaac6c8c8969239cab00848f6291454615a912`（TTS-C.1A.R7）；compose / Dockerfile / production env **零修改**（`git diff 37eaac6..01f8536 -- docker-compose.production.yml docker-compose.production.gpu.yml docker-compose.yml Dockerfile` 空）；仅 `.env.production` 中 `ZHIYING_RELEASE_TAG` 由 `37eaac6…` → `01f8536…`（README 明确这是部署锚点，不属 violation）；compose 配置解析输出 `@37eaac6` 与 `@01f8536` 逐字节一致。
- **Review 结论（部署前）**：**TTS-C.1C.1 Independent Review = PASS**；**TTS-C.1B.1 R1 blocker-specific Review = PASS**；**Integrated exact-SHA Review = PASS**。
- **pre-deployment backup**：`/vol1/1000/backups/zhiying/tts-c1b1-c1c1-20260806T063951Z-37eaac6c8c8969239cab00848f6291454615a912`（50 文件，47 项 SHA256SUMS 全 OK：`backup-meta.txt` 含 STAMP/RUNTIME_TAG/CHECKOUT_SHA/PREVIOUS_RUNTIME/TARGET_SHA/POST_GATE=not-set/INTEGRITY=ok/FK_CHECK_EMPTY=YES/ADAPTER_HEALTH_READY=true/VOICE_REGISTRY_SHA256=1dab4a31…/VOICE_REFERENCE_DEFAULT_V1_SHA256=2d85800…/TTS_JOBS_INSERT_DUMP_SHA256=780b024f…/PUB_ACT=0 REG_PUB=0 LEGACY=0 MAT_REQ=0 MAT_JOB=0 MAT=0 VPROF=0 VREV=0/TABLES=29 TRIGGERS=49 INDEXES=7/WEB_IMAGE=dd37ee1b7e56 ADAPTER_IMAGE=0ad9789bb9bb/COMPOSE_INVOCATION=...`；DB online backup via `sqlite3 .backup`；materialization inventory 0 文件 0 symlink；production voice-registry.json + voices/default-v1.wav 字节级 SHA-256 留底）。
- **Exact-SHA 构建**：`scripts/production-build-network.sh start`（CONNECT tunnel RUNNING）→ `docker build --network=host --add-host remotion.media:127.0.0.1 --build-arg APT_MIRROR=mirrors.aliyun.com --build-arg NPM_REGISTRY=https://registry.npmmirror.com -t zhiying:01f8536…`（BUILD_EXIT=0，多层 CACHED，仅末层 COPY 新代码）→ 镜像 ID `a5e20cfed36f`（2.56GB）；adapter build（独立 build 上下文避免 web 上下文污染）→ `docker build -t zhiying-indextts2-adapter:01f8536…`（BUILD_EXIT=0；Python deps 与上一 runtime 同型号：fastapi 0.115.6 / uvicorn 0.32.1 / pydantic 2.13.4，仅 server.py 内容更新）→ 镜像 ID `4e8379d9ca31`（135MB）；构建后 `scripts/production-build-network.sh stop`（tunnel-remotion-build 容器已清理，STOPPED；7890 host proxy 保留）。**web/worker image 零 Python 安装**——基础镜像仍为 `node:22-bookworm`，Dockerfile 未改，未安装 `python*` 包；adapter 独立镜像用 `python:3.12-slim` 是既有设计（M4-B），本轮未改 Dockerfile。
- **up 验证**：三容器 healthy @`zhiying:01f8536…` + `zhiying-indextts2-adapter:01f8536…`；worker `depends_on: adapter condition: service_healthy` 启动序列正常通过；restartCount 全部 0；web / worker / adapter 日志扫描无 error/exception/migration/recovery/heartbeat/path 异常；adapter 启动后首次 `GET /health` 200 = ready；无 reload 活动。
- **Smoke（production，只读 + 既有 production voice）**：
  - `GET http://127.0.0.1:3210/` → **200** text/html
  - `GET http://127.0.0.1:3210/api/projects` → **200** JSON（3 个 m6 项目）
  - `GET http://127.0.0.1:9880/health` → **200** `{ready:true, provider:indextts2, model:"IndexTTS-2", degraded:false}`（1B.1 在 `/health` 上加 `degraded` 字段以兼容 LKG degraded；本部署 cold-load 完整正确 → degraded=false）
  - `GET http://127.0.0.1:9880/registry-status` → **200** `{ready:true, degraded:false, schemaVersion:"1.0", loadedRegistrySha256:"1dab4a313689736aad081b2b0367f9b036eeec0f9a751647192498ce9ee21827", loadedRegistryGeneration:null, publisherSchemaVersion:null, speakerCount:1, detail:null, lastReloadError:null}`（cold-load legacy 1.0 registry；`generation/publisherSchemaVersion=null` 是 1.0 内部语义，1B.1 双 schema 兼容；`loadedRegistrySha256` 逐字节匹配 production registry 文件 SHA-256）
  - `POST http://127.0.0.1:9880/v1/synthesize {text:"smoke", voiceProfile:"default", voiceRevision:"1", useRandom:false, emotion:"none"}` → **200** audio/wav PCM mono 22050Hz **59470 B**（既有 voice + 最短 fixed text，registry / DB / IndexTTS2 行为零变）
  - `POST /api/projects/<id>/voice-materializations` → **503** `MATERIALIZATION_NOT_ENABLED`（POST 仍 disabled）
  - `GET /api/projects/<id>/voice-materializations` → **200** `{"requests":[],"adapterReady":false}`（GET 不受影响；`adapterReady:false` 为 1A 既有 placeholder，未被 1B 改 shape）
- **Reference validation**：每个 voice 单独 verify。production registry `default-v1`：path `/voices/default-v1.wav` ⊂ `/voices` containment root ⇒ contained=true；文件类型 RIFF WAVE PCM 16-bit mono 24000Hz；普通文件 + readable OK；实际 SHA-256 `2d85800…c73a30` == registry `referenceSha256` `2d85800…c73a30` ⇒ 1B.1.R1 reference 文件验证前置通过；文件 mtime/size/inode 不变 ⇒ **reference 未修改**；adapter cold-load 后 `loadedRegistrySha256` == production registry 文件原始 bytes SHA-256 (`1dab4a31…`) ⇒ adapter 完整识别既有 registry。
- **Production 数据不变量（before / after zero-change）**：integrity ok → ok；FK empty → empty；tts_jobs 行数 351 / INSERT dump SHA `780b024f…` → 351 / `780b024f…`（INSERT dump 字节相同，per-table CSV byte-compare 全部 PASS）；六张 TTS-C 表（voice_materialization_requests/jobs/voice_materializations/legacy_adapter_voice_entries/voice_registry_publications/voice_registry_publication_activations）=0 → =0；voice_profiles/revisions=0 → =0；其他 19 张业务表 counts 与 baseline 一致；materialization root 0 文件 0 symlink → 0 / 0；POST gate `not-set(disabled)` → **`not-set(disabled)`**（POST 503 实证）；migration/schema 无变化（§2 SQL SHA `c88f64ac…` 不变，1B.1 + 1C.1 零 schema 迁移）。
- **边界（per task contract）**：✅ production POST remains disabled（`TTS_C1A_MATERIALIZATION_POST_ENABLED` 未设置；POST 503 实证）；✅ production `POST /reload` was not called（adapter cold-load 直接读既有 1.0 registry，不触发 reload endpoint）；✅ production registry was not modified（mtime/size/inode/SHA 不变；file bytes 0 改动）；✅ reference voice file 未修改（mtime/size/inode/SHA 不变）；✅ migration / schema / trigger 未变化（frozen §2 SQL SHA 不变）；✅ production DB 数据未变化（per-table CSV byte-compare zero diff；tables/triggers/index counts 不变）；✅ publication / activation / legacy 表保持 0 行；✅ TTS-C.1B.2 / 1B.3 not started；✅ TTS-C.1C.2 not started；✅ TTS-C.2 not authorized；✅ compose / Dockerfile / production env 文件未修改（git diff 旧→新空，唯一 env 改动：`ZHIYING_RELEASE_TAG` 锚点按 README）；✅ Node web/worker image 未安装 Python（基础镜像仍 `node:22-bookworm`，Dockerfile 未改）；✅ 完整 M7 gate 未在 production Node image 内运行（已在 agentvm 完成）。
- **状态**：**M7.3B FROZEN；TTS-A FROZEN；TTS-B FROZEN；TTS-C.0 FROZEN；TTS-C.1A = FROZEN；TTS-C.1B.1 = FROZEN；TTS-C.1C.1 = FROZEN；production runtime = `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；production POST remains disabled；TTS-C.1B.2 / 1B.3 / 1C.2 not started；TTS-C.2 not authorized；Deployment Evidence Review = PASS**。
- **证据边界**：本次 production 部署为 docs-only evidence（无新 GitHub Actions run 触发——本轮 1B.1 + 1C.1 在 agentvm 的 CI 证据已通过 integrated exact-SHA Review 完成，部署本身为同一 SHA 的 runtime 切换）。production 部署证据由 Agent 现场命令采集并备份到 `/vol1/1000/backups/zhiying/tts-c1b1-c1c1-20260806T063951Z-37eaac6c8c8969239cab00848f6291454615a912`；OPS-AUDIT-BRIDGE 不可用，**不是** independently verified（与历次 1A 部署同口径）。
- **Deployment Evidence Review closure（审计链，2026-08-06）**：
  - 初次 Deployment Evidence Review = **FAIL**（evidence provenance 错写：原 evidence 将 agentvm local gate 错写为 GitHub Actions independent evidence）。
  - R1 仅修正文档 provenance 与阶段状态措辞（`docs/evidence/tts-c-r17/deployment-c1b1-c1c1.md`「证据口径」段修正为：branch-specific suites 与 integrated full gate 由 agentvm 本地执行，Integrated exact-SHA Review 已核对两个 reviewed 分支的核心 blob identity、集成范围、gate 接线及本地 54/54 执行报告；exact runtime SHA `01f8536…` 未产生新的 GitHub Actions workflow run / commit status，不得表述为 GitHub-hosted independent CI evidence；Final state 修正为 `TTS-C.1B.2 / 1B.3 not started`、`TTS-C.1C.2 not started`、`TTS-C.2 not authorized`）。R1 docs SHA = `b5b97805a90d1a80b4f42e597fd1ba76aa4480de`。
  - runtime deployment 本身没有失败信号：没有 rollback，没有 redeploy，production 未被访问。
  - **Deployment Evidence R1 blocker-specific Review = PASS**。
  - **Deployment Evidence Review = PASS（最终结论）**；TTS-C.1B.1 = FROZEN；TTS-C.1C.1 = FROZEN。

### 15.17 TTS-C.1B.2 部署证据（6874f51，2026-08-06，Deployment Evidence Review = PASS → TTS-C.1B.2 FROZEN）

> 惰性 library/runtime 更新：只替换 web/worker；adapter 不重建不重启；不执行任何业务入口。
> 详情：`docs/evidence/tts-c-r17/deployment-c1b2.md`（同 commit 独立 evidence 文档）。

- **部署链**：deployed SHA `6874f51c717ebab1c282ee29e9301f27627deaf7` = production runtime SHA =
  host checkout SHA = `ZHIYING_RELEASE_TAG`；previous runtime `01f8536b4bac1661aa86ad57f90985ec56c8aaa5`；
  backup `/vol1/1000/backups/zhiying/tts-c1b2-20260806T121853Z-01f8536b4bac1661aa86ad57f90985ec56c8aaa5`
  （27 文件，SHA256SUMS 25 项全 OK；DB online backup integrity=ok、FK empty）。
- **构建**：production-build-network start（RUNNING）→ `docker build --network=host
  --add-host remotion.media:127.0.0.1 --build-arg APT_MIRROR --build-arg NPM_REGISTRY
  -t zhiying:6874f51…`（BUILD_EXIT=0）→ image `c215821a21bc`（2.56GB）；build 后 tunnel stop
  （STOPPED）。Node image 无 Python 变更（Dockerfile 未改）。adapter 镜像未构建。
- **up（仅替换 web/worker）**：`docker compose -f docker-compose.production.yml -f
  docker-compose.production.gpu.yml --env-file .env.production up -d --no-deps zhiying-web
  zhiying-worker`（UP_EXIT=0）；web/worker 新 image `zhiying:6874f51…`（c215821a21bc）healthy；
  adapter container id/image/startedAt 完全不变（`bf6a70e3d25d` / `4e8379d9ca31` /
  2026-08-06T06:46:48Z）——未重建未重启；compose topology / mounts / Dockerfile / secrets / proxy
  零修改；唯一 env 变更 = `ZHIYING_RELEASE_TAG`（env before `aa8f85cc…` → after `5a4e825b…`，
  逐行 diff 仅此一行）。
- **Smoke（production）**：`GET /api/projects` 200；`GET /` 200；adapter `/health` 200
  （ready/degraded=false）；adapter `/registry-status` 200（schemaVersion 1.0、loadedRegistrySha256
  `1dab4a31…`、generation/publisherSchemaVersion null、speakerCount 1、lastReloadError null——
  与 preflight 逐字节一致）；POST materialization → **503 MATERIALIZATION_NOT_ENABLED**；
  synthesis smoke（`default@1` + `smoke`）→ 200 audio/wav 75342 B；web/worker 日志无
  migration/constraint/publisher/import/candidate/reload/unhandled 异常；adapter 日志无 reload 活动。
- **Zero-change（preflight vs post）**：TTS-C 六表 count 0→0 + `.mode insert` export dump
  byte-identical；tts_jobs 351→351 + dump byte-identical（hash `780b024f…`）；integrity ok / FK
  empty；active registry（322B/inode 71982/SHA `1dab4a31…`）与 reference voice（299052B/inode
  68221/SHA `2d85800…`）size/mtime/inode/SHA 全不变；candidate root `data/voice-registries`
  pre/post 均 **ABSENT**（无 candidate-<generation>.json 创建）；adapter registry-status identity
  不变（未被 reload）。
- **证据边界**：deployment gate 在 Feiniu production 现场执行（命令级证据持久化到 backup）；
  development tests 与 55-suite gate 在 agentvm 执行（`QUALITY_GATE_RESULT=PASS`、
  `QUALITY_GATE_TOTAL_SUITES=55`、`QUALITY_GATE_SHA=6874f51c717ebab1c282ee29e9301f27627deaf7` ==
  deployed SHA）；**该 SHA 没有 GitHub Actions workflow run/commit status——不得写成 GitHub-hosted
  independent CI evidence**；production 证据 OPS-AUDIT-BRIDGE 不可用，非 independently verified。
- **状态**：**M7.3B FROZEN；TTS-A/B/C.0/C.1A/C.1B.1/C.1C.1 FROZEN；TTS-C.1B.2 deployed；
  production runtime = `6874f51c717ebab1c282ee29e9301f27627deaf7`；production POST remains
  disabled；**Deployment Evidence Review = PASS（2026-08-06）→ TTS-C.1B.2 = FROZEN**；
  TTS-C.1B.3 authorized（not yet implemented）；TTS-C.1C.2 not started；TTS-C.2 not authorized**。
- **非阻断说明（2026-08-06）**：未来 deployment restart count 使用 `docker inspect` 顶层
  `.RestartCount`；本轮（1B.2）adapter 未重启由 container ID + image ID + StartedAt +
...
### TTS-C.1B.3 initial implementation on work branch（initial Independent Review = FAIL；not merged；not deployed）
...
### TTS-C.1B.3.R1 blocker repair（R1 blocker-specific Review = FAIL；R2 pending；not merged；not deployed）

- **P0-A fenced enterIndeterminate**：`enterIndeterminateFenced`（单 statement：id + status==fromStatus +
  owner_token + attempt + DB_NOW<=lease + indeterminate_from_status IS NULL → indeterminate +
  owner/lease=NULL + error_code/message；changes!=1 → PUBLICATION_NOT_OWNER）。indeterminate 自动入口
  只允许 fromStatus='activation_pending'（类型层 + 运行时）。真实竞态测试（R1-01）：A poll 挂起
  （delay-close mock）→ A lease 过期 → B takeover → A 的 enterIndeterminate fence 失败，B
  owner/attempt/lease 不变、publication 不 indeterminate、A 无后续副作用。
- **P0-B file_durable reload 不确定可恢复闭环**：reload timeout/reset/malformed 不再进入
  indeterminate——保持 file_durable，执行一次 fenced registry-status 观察（candidate → markActivationPending
  + T5；stable → reload_retryable；unknown/status 不可用 → 新 outcome `reload_result_unknown` 保持
  file_durable 等 recovery，不释放 single-flight）。测试 R1-02/03/04 + A6 更新；原 F5
  （file_durable → 永久 indeterminate）删除。
- **P0-C active registry rollback 闭环**：`durabilizeStableSnapshot`（首次 promotion 前持久化
  `<dataDir>/voice-registries/stable-before-<generation>.json`；同 SHA re-durabilize 复用/异 SHA
  STABLE_SNAPSHOT_CONFLICT；raw durability——stable 是 1.0 格式）；`restoreStableAndConfirm`
  （disk candidate → snapshot 原子恢复 → reload → registry-status loaded==stable 确认；不确定 →
  rollback_pending/unknown，不释放 single-flight）。测试 R1-05/06/07（含冷启动模拟：adapter 重启
  重读 active path 后 loaded 仍 stable）。
- **Legacy rollback**：`failPublicationAndRollbackLegacy` / `cancelPublicationAndRollbackLegacy`（单
  BEGIN IMMEDIATE：publication fenced terminal + legacy subject mapping_pending→mapped_verified 清
  pending link/selector；legacy changes!=1 → 整事务回滚；mapping_mode/provenance 保持；
  materialization_publish/registry_rebuild 不触碰 legacy）。测试 R1-08（cutover_publish）/R1-09
  （cutover_existing）含 rollback 后新 publication 可创建。
- **P1-A per-side-effect renew**：`renewPublicationOrThrow`（每个 HTTP 前）；reconcileActivationPending
  的 status/reload/第二次 status 各自 fenced renew；publishRegistryCandidate(handle) 的 file_durable
  re-durabilize 前 renew。测试 R1-10（reload 挂起期间 takeover → 原 owner 不再执行第二次 status）。
- **P1-B active path 全链 containment**：`validateActivePathSafe`（root 绝对/非 symlink/realpath 固定 +
  final lexical 在 root 内 + 逐级 parent 无 symlink + final O_NOFOLLOW fstat regular），read/classify/
  promotion（含 candidate-idempotent 路径）/restore 全部前置执行。测试 R1-11（root symlink + disk
  candidate）/R1-12（nested parent symlink）。
- **P2 adapter error contract**：client 优先解析真实 shape `{"error","message"}`（兼容嵌套结构）；
  mock 改用真实 shape；测试 R1-13（errorCode==VOICE_REGISTRY_RELOAD_FAILED + message 含底层错误）。
- **测试**：`scripts/test-tts-c1b3-activation-recovery.ts` **149 PASS ×2 独立运行**（原 107 +
  R1 新增 42；mock HTTP adapter + 真实双进程 takeover + delayed-mock 竞态 + 故障注入；零 production
  零真实 IndexTTS2）。1B.2 138 PASS ×2 保持绿；combined gate suite **56**。
- **未执行**：production 一切操作；TTS-C.1C.2；TTS-C.2。production runtime 保持 `6874f51…`；
  production POST remains disabled。

### TTS-C.1B.3.R2 blocker repair（pending blocker-specific Review；not merged；not deployed）

R1 已关闭的 fenced indeterminate、file_durable uncertainty、adapter error contract、path containment
全部保留；本轮只修 Independent Review 确认的四个 blocker：

- **P0-1 active 文件 mutation 紧邻前重新 fencing**：任何真正修改文件的操作前紧邻执行
  `renewPublicationOrThrow(db, publicationId, ownerToken, attempt, '<exact step>')`，覆盖：
  stable snapshot 新文件写入前 / snapshot existing re-durabilize 前 / candidate→active
  `atomicWriteFile` 前 / stable→active `atomicWriteRawFile` restore 前。挂起 hook
  （`beforeActiveWriteHook` / `beforeStableRestoreWriteHook` / `beforeSnapshotWriteHook`，仅测试注入）
  在最终 renew 之前触发——stale owner 在最终 renew 处失败，renew 与 mutation 之间不再执行任何
  HTTP/其他文件写入/可阻塞 hook。测试 R2-01（promote 竞态：A 挂起 → lease 过期 + B takeover → A
  PUBLICATION_LEASE_EXPIRED，active registry bytes 不变、B owner/attempt/lease 不变、无 temp）、
  R2-02（restore 竞态 → rollback_pending，disk 不被覆盖）、R2-03/03b（snapshot 新写与
  re-durabilize 竞态 → 不创建/不 fsync snapshot）。
- **P0-2 indeterminate→failed atomic legacy rollback**：新增 `resolveIndeterminateFailedAndRollbackLegacy`
  （无 owner；单 BEGIN IMMEDIATE：publication indeterminate→failed（id + status='indeterminate' +
  indeterminate_from_status='activation_pending' + attempt 精确匹配）→ legacy subject 同事务
  mapping_pending→mapped_verified（清 pending link/selector；mapping_mode/provenance 保持）；
  legacy changes!=1 → LEGACY_ROLLBACK_MISMATCH 整事务回滚，publication 保持 indeterminate）。
  `reconcileIndeterminate()` 在 disk+adapter 均 stable 时改调此 helper，不再直接 UPDATE failed。
  测试 R2-04（cutover_publish：indeterminate→failed + rollback + 可创建新 publication）、R2-05
  （cutover_existing：projection published evidence 零改写）、R2-06（人为破坏 pending link →
  整事务回滚，publication 保持 indeterminate；schema trigger 防篡改使该状态只能以直接 SQL 构造）。
- **P1-1 building/candidate_persisted legacy rollback**：terminal rollback 拆分两个显式入口——
  `failPrePromotionPublicationAndRollbackLegacy`/`cancelPrePromotion…`（building/candidate_persisted，
  active registry 尚未被本 publication 提升，无需 stable restore）与
  `failPostPromotionPublicationAndRollbackLegacy`/`cancelPostPromotion…`（file_durable/activation_pending，
  调用方必须已 restoreStableAndConfirm=='confirmed'）；R1 的 `failPublicationAndRollbackLegacy` 保留为
  post-promotion 兼容入口。测试 R2-07（building legacy failure → terminal + rollback + 可重试）、
  R2-08（candidate_persisted legacy failure）、R2-09（legacy link 不匹配 → 整事务回滚，publication
  保持 building）。
- **P1-2 exact final parent directory fsync**：`durabilizeRawFile` 与 `durabilizeAndVerifyCandidate`
  的 dir fsync 目标改为 `path.dirname(finalAbs)`（rename 实际发生的目录；lstat 校验 parent 非 symlink
  且为目录 + O_NOFOLLOW 打开），不再无条件 fsync containment root；candidate 文件为 root 直接子文件，
  1B.2 行为不变（138 断言保持绿）。测试 R2-10（nested `root/sub/voice-registry.json`：fsync 目标
  = `root/sub` 而非 `root`（/proc/self/fd 解析实证）；exact parent fsync 注入失败 → restore
  rollback_pending / promote CANDIDATE_FILE_IO，零 reload 零 activation）。
- **测试**：`scripts/test-tts-c1b3-activation-recovery.ts` **214 PASS ×2 独立运行**（原 149 +
  R2 新增 65；R2-01…R2-10 + R2-03b；挂起 hook 竞态 + 直接 SQL 故障构造 + fsync 目标实证；零
...
### TTS-C.1B.3.R4 exported-API closure（**R4 Final Blocker-Specific Review = PASS**；not yet deployed；not yet frozen）

完整审查链：initial Independent Review = FAIL → R1 blocker-specific Review = FAIL → R2
blocker-specific Review = FAIL → R3 final blocker-specific Review = FAIL → R4 exported-API
closure → **R4 Final Blocker-Specific Review = PASS**。reviewed implementation SHA =
`aad06c0c89ffc795c366df6f98c5eba54cffa4f1`。TTS-C.1B.3 READY FOR INTEGRATION TO m7；
not yet deployed；not yet frozen。

**R3 final blocker-specific Review = FAIL 原因**：`src/lib/tts-c/registry-publisher.ts` 仍公开导出
`failPublication()`，允许 building/candidate_persisted/**file_durable** 直接进入 failed，可绕过
stable restore、adapter acknowledgment 和 legacy rollback。R4 关闭该剩余 P1 blocker。

- **仓库级调用点审计**（rg 全仓）：`failPublication` 定义于 `registry-publisher.ts`（已移除）；
  import 点：`registry-activation.ts`（未使用，已删除）、1B.2/1B.3 测试（测试清理用途，已迁移到
  测试本地等价 SQL helper `releasePublicationForTest` / `failRow`——非公开 API）；worker 无引用；
  production 代码零调用。
- **移除公开绕过 API**：`FailPublicationOptions` + `export function failPublication` 从
  `registry-publisher.ts` 公开 API 完全移除（方案 A）。最终仓库不存在任何公开 DB-only helper
  可将 file_durable/activation_pending 直接推进 failed/cancelled。错误路径迁移：
  building/candidate_persisted + legacy → `fail/cancelPrePromotionPublicationAndRollbackLegacy`
  （atomic legacy rollback）；building/candidate_persisted + non-legacy → 同一 pre-promotion
  helper（non-legacy 分支不触碰 legacy 表）；file_durable/activation_pending →
  `fail/cancelPostPromotionPublicationSafely`（restore + reload ack 强制前置）；indeterminate →
  `resolveIndeterminateFailedAndRollbackLegacy`（仅 reconcileIndeterminate）。未新增任何公开宽泛
  terminal helper。
- **测试**：`scripts/test-tts-c1b3-activation-recovery.ts` **259 PASS ×2 独立运行**（原 242 +
  R4 新增 17：R4-01 file_durable 不能直接 fail——编译期 `@ts-expect-error`（符号存在则 tsc 报
  unused error）+ 运行时 `in` 检查证明 `failPublication` 已移除，safe orchestrator 完成安全
  rollback；R4-02/06 导出清单审计——两个模块 6 个 banned 符号均不存在，导出清单打印为审计证据；
  R4-03 legacy building / R4-04 legacy candidate_persisted → 只能走 pre-promotion legacy
  rollback（failed + mapped_verified + link/selector 清空）；R4-05 non-legacy building → failed
  （不触碰 legacy），file_durable → pre-promotion helper 拒绝（PUBLICATION_NOT_OWNER，保持
  file_durable））。1B.2 138 PASS ×2 保持绿（release 路径迁移不改变断言语义）；1B.1 34 / 1C.1 /
  m4b 22 / TTS-C.1A 九套回归全绿；combined gate suite **56**。
- **状态**：R4 Final Blocker-Specific Review = PASS；ALL IDENTIFIED 1B.3 BLOCKERS CLOSED；
...
### TTS-C.1B.3 Deployment Topology R2（2026-08-07；Materialization Rollback Procedure Closure；**R2 Final Topology-Specific Review = PASS**）

**Topology R1 Review = FAIL 原因**：rollback plan 未包含 materialization 反向 relocation——
恢复旧 release/compose 后，旧 runtime 继续从 `OLD_MAT_ROOT`
（`data/voice-materializations`）读取，而新 topology 的 materialization tree 位于
`NEW_MAT_ROOT`（`voices/tts-a`），旧 runtime 将看不到数据。R2 为 **docs-only** closure：
- **完整 rollback 顺序**（§M2.6）：quiesce writers（停 web/worker、确认 worker 停止、记录
  rollback 前 NEW inventory）→ NEW_MAT_ROOT inventory（relative path/type/size/SHA-256 +
  lstat/realpath/no-symlink-escape；**即使当前为空也必须走同一 procedure**，空目录只是事实不是
  contract）→ OLD_MAT_ROOT collision check（不存在→创建 parent；空→允许但明确处理；非空→与
  NEW inventory 完全一致才允许幂等策略，否则 FAIL；禁止 rm -rf/覆盖/静默合并）→ reverse
  relocation（同 filesystem（dev=40 实证）优先 atomic rename，禁止长期第二份 mutable copy；
  临时 copy 需 fsync + inventory/SHA equality 后才 retire source）→ verification（before-NEW
  inventory == after-OLD inventory 逐项相等；NEW_MAT_ROOT 不再作为第二份 mutable tree）→
  registry rollback（旧 single-file topology + SHA `1dab4a31…` 确认）→ 恢复旧 compose/env +
  release tag `6874f51…` → 旧 runtime 启动前 OLD_MAT_ROOT 已恢复完整数据 → 启动 → smoke。
- **旧 runtime smoke**：web health / worker liveness / adapter /health + /registry-status
  （loaded == registry SHA == `1dab4a31…`）+ **materialization visibility smoke**（inventory
  非空：至少 1 个文件 host OLD_MAT_ROOT SHA == old worker 容器
  `/app/data/voice-materializations/<rel>` SHA；inventory 空：old worker 该目录存在且为空——
  **不得因空跳过 mount 验证**）。
- **Rollback boundary**：仅当 POST disabled + 无 publication/reload + 无 registry generation
  推进时，旧 single-file topology 才是 rollback target；一旦发生新 publication activation /
  registry rename update / 需新 topology 的 materialization 状态迁移，旧 topology 不再是普通
  rollback target，必须走单独 recovery / forward-fix procedure。
- **部署前备份要求**：备份 compose/.env.production/release tag/docker inspect mount map，记录
  路径与 SHA/permissions；secret 不得写入 repo/evidence。
- 修改范围：仅两份 docs（`docs/TTS_C_1B_1C_EXECUTION_PLAN.md` §M2.6、
  `docs/M7_IMPLEMENTATION_STATUS.md`）；未改 compose/.env/scripts/src/services/schema。
- 验证：`git diff --check` / `--stat` / `--name-only`（仅两份 docs）；tsc --noEmit 通过。
  R1 既有证据保留：topology suite 39 PASS ×2、1B.3 267 PASS ×2、R1 exact-SHA gate 56/56 PASS。
- **状态**：R2 Final Topology-Specific Review = PASS；ALL IDENTIFIED DEPLOYMENT TOPOLOGY
  BLOCKERS CLOSED；READY FOR TOPOLOGY INTEGRATION TO m7；reviewed topology SHA
  `6f2f1ede5e72974ed36efd5a5cd2f8cb58f21d47`；NOT DEPLOYED；NOT FROZEN。未执行：production
  一切写操作（含 directory move、.env 修改、compose up/restart、registry write、/reload、
  DB write、synthesis）；TTS-C.1C.2；TTS-C.2。production runtime 保持 `6874f51…`；
  production POST remains disabled。

### TTS-C.1B.3 Deployment Topology Closure R1（2026-08-07；配置闭环；R1 历史记录；原始三个 blocker 已关闭）
...
deployment）：实施 frozen plan 已明确的 topology 变更并提交独立 Topology Review。**未 SSH 修改
production；未 compose up/build；未重启任何容器；未写 registry；未 POST /reload；未执行
publication/import/synthesis。**

- **Topology 变更（frozen 两项要求关闭）**：
  - **A. active registry：single-file bind → directory mount**。旧
    `${ZHIYING_HOST_VOICE_REGISTRY}:/config/voice-registry.json:ro` 退役（单文件 bind 目标
    inode 不能安全作为 publisher `temp→fsync→rename` 的 replacement）；新
    `${ZHIYING_HOST_REGISTRY_DIR}:/config:ro`（adapter）与
    `${ZHIYING_HOST_REGISTRY_DIR}:/app/data/registry:rw`（worker）——同一 host 目录，
    adapter 固定路径 `ADAPTER_VOICE_REGISTRY_PATH=/config/voice-registry.json` 保持。
  - **B. materialized references 对 adapter 可见**：adapter 新增
    `${ZHIYING_HOST_MATERIALIZATIONS_DIR}:/voices/tts-a:ro`（非重叠子命名空间；
    `ZHIYING_EMIT_VOICE_ROOT_PATH=/voices/tts-a`）；worker legacy voices 新增
    `${ZHIYING_HOST_VOICES_DIR}:/voices:ro`（registry 文档 `/voices/x` 直接解析，零 runtime
    修改）。未把 /app/data 暴露给 adapter；adapter 全 ro；无 privileged/docker.sock。
  - **Worker 1B.3 recovery controller 显式接线**：
    `ZHIYING_ACTIVE_REGISTRY_PATH=/app/data/registry/voice-registry.json`、
    `ZHIYING_ACTIVE_REGISTRY_ROOT=/app/data/registry`、
    `ZHIYING_LEGACY_VOICE_ROOT_DIR=/voices`、`ZHIYING_EMIT_VOICE_ROOT_PATH=/voices/tts-a`；
    adapter URL 复用既有 `INDEXTTS2_BASE_URL=http://127.0.0.1:9880`。
- **POST / business 入口保持关闭**：无自动 publisher/import/publication/activation/reload/
  synthesis；preflight 实证 production DB publications=0/activations=0/legacy=0——recovery
  sweep 为 no-op（zero publication → zero registry mutation）。
- **只读 production preflight（2026-08-07）**：registry
  `/vol1/1000/docker/zhiying/voice-registry.json`（regular, 322B, SHA
  `1dab4a313689736aad081b2b0367f9b036eeec0f9a751647192498ce9ee21827`）→
  `/config/voice-registry.json:ro`；voices `/vol1/1000/docker/zhiying/voices` → `/voices:ro`；
  materializations `/vol1/1000/docker/zhiying/data/voice-materializations` →
  `/app/data/voice-materializations:rw`（worker 唯一）；adapter /health ready=true、
  /registry-status 1.0/SHA 匹配/speakerCount=1；DB integrity ok / FK 0 / publications 0 /
  activations 0 / legacy 0 / tts_jobs 351；容器 ID/StartedAt/RestartCount 已记录（adapter
  未重启：restart=0）。
- **Host migration procedure + rollback plan**：零内容变化迁移（backup → 记录 SHA → 创建
  `voice-registry/` 目录 → copy → fsync 文件+目录 → SHA 相等 → lstat/realpath 校验 → 才改
  compose/env）与可执行 rollback（回旧 single-file topology + 旧 release tag + 原始 SHA
  确认）完整写入 `docs/TTS_C_1B_1C_EXECUTION_PLAN.md` §M2.5/M2.6；本轮不执行迁移。
- **测试**：`scripts/test-tts-c1a-compose-mounts.ts` 重写为 **36 PASS ×2**（CM-01…06 保留 +
  TOP-01…09：registry 目录 RO/RW、host source identity（文本 + `docker compose config` 渲染
  双层）、legacy voices RO、materialization 可见性、permissions 矩阵、无 single-file bind、
  recovery env completeness、渲染后 resolved volumes/env 一致）。1B.1/1B.2/1B.3 与 TTS-C.1A
  九套回归全绿；combined gate suite **56**。
- **agentvm gate intermittency（非阻断观察，独立 test-infra debt）**：integration candidate
  gate 曾出现跨不同 TTS-C.1A suite 的 staging EOF（ffmpeg 读 `.staging/original.bin` 报
  `End of file`；失败 suite 各不同：recovery/worker/r6-hardening）；同一 exact SHA 后续
  candidate gate + integrated m7 gate 均 56/56 PASS；失败 suite 单独重跑全部通过；60×ffmpeg
  与 30×完整 ingest 压力测试 0 失败；内存/磁盘/inode/fd/负载监控正常；无残留进程；相关 commit
  为 docs-only。**exact root cause 未证明；不归因于 1B.3 runtime；保留为独立 test-infra
  debt；不得宣称已 root-caused**。
- **未执行**：production 一切写操作；TTS-C.1C.2；TTS-C.2。production runtime 保持
  `6874f51…`；production POST remains disabled。

### TTS-C.1B.3 Production Deployment（2026-08-07；**PRODUCTION DEPLOYMENT = PASS → Deployment Evidence Review = PASS → TTS-C.1B.3 = FROZEN**）

- **授权链**：Final Blocker-Specific Review PASS → Integrated exact-SHA Review PASS →
  Deployment Topology R2 Final Topology-Specific Review PASS → Integrated Topology exact-SHA
  Review PASS → **PRODUCTION DEPLOYMENT AUTHORIZED**。
- **部署结果**：production runtime 由 `6874f51c717ebab1c282ee29e9301f27627deaf7` 切换到
  `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`（三容器 exact tag；web/worker image
  `ead89ff3b4a3`、adapter image `6e6c504bf5af`；restart 全部 0）。topology 切换：
  registry single-file bind → 目录 mount（`/vol1/1000/docker/zhiying/voice-registry`：
  worker `/registry:rw`、adapter `/config:ro`）；materialization host root 迁移到
  `voices/tts-a`（空目录 same-fs mv，inode 保留）；adapter 唯一 `/voices:ro` mount（无 child）；
  worker recovery controller env 显式接线（`/registry/voice-registry.json`、`/voices`、
  `/voices/tts-a`）。registry bytes/SHA 不变（`1dab4a31…`）；adapter loaded SHA == host SHA。
- **零副作用证据**：DB integrity ok / FK 0 / publications 0 / activations 0；worker log
  "tts-c1b3 registry recovery controller started"；无 candidate/snapshot/voice-registries；
  无 `/reload`；production POST remains disabled（503 + env flag 未设）；无真实 synthesis；
  其他宿主容器未受影响。materialization forward migration + registry directory migration
  + rollback 依据（§M2.5/M2.6）完整记录于 `docs/evidence/tts-c1b3/deployment.md`。
- **证据边界**：deployment gate 在 Feiniu production 现场执行；build 使用既有
  `scripts/production-build-network.sh` 隧道（M7.3A.3）+ APT_MIRROR/NPM_REGISTRY build args
  （1B.2 同款）；完整 56-suite gate 在 agentvm 执行（docs commit 前后均有 exact-HEAD PASS）；
  无 GitHub-hosted CI 证据。production runtime 权威 = deployed exact SHA
  `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`（≠ evidence docs commit SHA）。
- **状态**：**TTS-C.1B.3 = FROZEN**（审计链末尾：Production deployment PASS → Deployment
  Evidence Review = PASS → FROZEN；frozen production runtime =
  `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；deployment evidence docs commit =
  `c4dc544642cf7e1ce54648430d8eee050c5651a1`——两者严格区分）。production POST remains
  disabled；TTS-C.1C.2 not started；TTS-C.2 not authorized。

### TTS-C.2 Phase 1 + TTS-C.1C.2（2026-08-07；work branch `work/tts-c2-phase1`；PENDING R1 BLOCKER-SPECIFIC REVIEW）

- **授权**：TTS-C.2 = NOW AUTHORIZED FOR IMPLEMENTATION（Phase 1：C.2 orchestration / persistence /
  payload contract；1C.2 provenance / payload-builder integration 随 C.2 payload builder 一次性完成，
  不再作为独立空壳 milestone）。TTS-C.1B.3 = FROZEN；production runtime 保持
  `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3`；production POST remains disabled。
- **Frozen C.2 migration**（`scripts/build-tts-c2-migration.ts` + `src/lib/tts-c/migration-c2.generated.ts`
  + `migration-c2.ts`；唯一来源 = frozen 设计文档 §2，与 1A 同一提取/幂等机制）：
  `tts_audio_requests` / `tts_synthesis_claims` / `tts_claim_generation_dispatches` /
  `tts_job_execution_transitions`（第 13 表）/ `tts_generation_attempts` / `sentence_audio_artifacts`
  + `tts_jobs` additive migration（10 条 ADD COLUMN 按列存在性动态跳过 + 3 unique index + 10 trigger；
  legacy 行双向 NULL 兼容，零 rebuild）。fresh / upgrade（1A + legacy 351 行）/ 幂等 / integrity /
  FK / legacy requeue 兼容全部测试实证。`getDb()` 自动应用 1A→C.2。
- **Orchestration**（`src/lib/tts-c/synthesis.ts` + `synthesis-execution.ts`）：
  request initializing→waiting 同事务 exact claim link；fan-in（many requests → one claim）；
  validating_reuse（validation owner/lease/attempt + candidate 查找）；usable → 复用（零 provider
  job）；unusable + subscriber>0 → 原子 dispatch（单条 command INSERT，frozen trigger 自动完成
  fenced 验证 + zero-subscriber 检查 + 恰好一个 queued job + claim→generation_pending）；
  unusable + subscriber=0 → claim cancelled；per-request cancel 只 detach 自己（最后 subscriber：
  validating_reuse → claim cancelled；generation_pending/running → job.cancel_requested=1）；
  validation lease takeover（attempt+1）；五类 execution command（worker_claim / lease_renewal /
  execution_takeover / prestart_terminal / state_transition）——claim/job 任何 status 与 owner
  字段迁移只能经 command（frozen per-column fence trigger 强制）；attempt phase 状态机 +
  write-once evidence；immutable artifact + provenance 闭包（narration/assignment/performance
  plan + voice + fingerprint 逐项与 job/claim/attempt 一致）。
- **1C.2**（`src/lib/tts-c/synthesis-payload.ts`）：复用冻结 1C.1 compiler；canonical synthesis
  payload（固定键序）+ `synthesisPayloadFingerprint`（sha256:canonical；确定性/敏感）；capability
  provenance 三件套（capability_snapshot_json / compiled_payload_json /
  capability_compiler_version）来自实际执行的 exact compilation（同事务写入，非事后重读）。
  `ttsInputFingerprint` 未改。
- **测试**：`scripts/test-tts-c2-synthesis.ts` **143 PASS ×2**（M1/M2 migration + R1-R16 场景）。
  1A migration 套件的 C.2 负向断言更新为 C.2 语义（C.2 表由 C.2 migration 提供）。gate suite
  **57**（新增 tts-c2-synthesis）。
- **未执行**：production 部署/迁移/POST enable/真实 synthesis/historical backfill；TTS-C.3+ 未启动。

### TTS-C.2 Phase 1 R1 blocker repair（2026-08-07；work branch `work/tts-c2-phase1`；R1 Blocker-Specific Review = FAIL（P0=0，P1=1）→ R2 closure implemented）

- **基线**：R0 reviewed SHA = `ce0bb5676a55a6c6b751aeddf632f2f08e71f250`；Independent Review = FAIL
  （P0=0，P1=4）。本轮仅修 4 个已确认 blocker，不重新设计 frozen R13 contract、不加表、不改
  frozen SQL、不扩展 Worker/Scheduler production wiring、不部署。
- **P1-1 atomic terminal closure**（`src/lib/tts-c/synthesis-execution.ts` `finalizeSynthesisJobSuccess`）：
  整个成功终局收敛为单个 BEGIN IMMEDIATE——fenced reread claim/job/attempt → active subscriber 重读
  （0 subscriber → `EXECUTION_ZERO_SUBSCRIBER` 整事务回滚，不 INSERT artifact / 不 succeeded）；
  逐一核对 active subscriber 的 frozen identity（project/unit/exact_source_fingerprint/
  synthesis_payload_fingerprint/final_tts_input_fingerprint/generation_variant_id）与
  claim/job 一致（否则 `REQUEST_STATE_INCONSISTENT`）；输出文件 exact reread（root containment +
  逐级 parent 非 symlink + O_NOFOLLOW + fstat regular + SHA/size 与 file_durable evidence 逐项一致）；
  source artifact（narration_plan_v2 / project_voice_assignment / narration_performance_plan）按
  artifacts 行 canonical JSON hash 重算并比对（不 trust caller 传入）；voice revision exact
  （profile/revision pair + canonical_audio_sha256 + canonical 文件同步 SHA 复核）；synthesis_payload_
  fingerprint 按 job payload 重算比对；attempt `file_durable→succeeded`（changes=1）；artifact
  INSERT + state_transition command 双侧；request fan-out（全部 waiting/running → succeeded +
  同一 result artifact；changes==activeCount，leftover==0 复查）。任何一步失败整事务回滚。
- **P1-2 validation fencing**（`src/lib/tts-c/synthesis.ts`）：`resolveClaimValidation` options 新增
  `candidateArtifactId?/candidateMetadataHash?`（NULL 用 SQLite `IS` 语义）；usable 与 zero-subscriber
  分支的最终 UPDATE 均为 frozen §3.1 exact fenced CAS（status + owner + attempt + `DB_NOW_MS <=
  lease` + candidate `IS` 双匹配），`changes===1` 否则 `VALIDATION_STALE_OWNER` 整事务回滚零副作用；
  新增 `renewClaimValidation`（frozen §3.3：status/owner/attempt exact + 旧 lease >= DB_NOW，changes=1
  才成功）；`takeoverClaimValidation` 同时刷新 `validation_started_at`（frozen §3.2）。
- **P1-3 attempt propagation**（`src/lib/tts-c/synthesis-execution.ts`）：`claimSynthesisJob` 读
  `claim.validation_attempt`（不再写死 1）→ command.worker_attempt / job.attempt /
  generation_attempt.attempt_number 三者一致；`prestartTerminalSynthesisJob` 的
  command.worker_attempt = claim.validation_attempt，但 job.attempt 保持 0（frozen D5：prestart
  无 Worker execution）。
- **P1-4 requestId replay/conflict**（`src/lib/tts-c/synthesis.ts` `createSynthesisRequests`）：
  scope = (project_id, request_id)；已存在且 frozen identity（unit_id + 四 fingerprint +
  generation_variant_id）全同 → replay（返回既有 request/claim 状态，不 INSERT、不建 claim、不抛
  raw UNIQUE）；不同 → `REQUEST_ID_CONFLICT`（不暴露 SQLite UNIQUE error）。
- **测试**：`scripts/test-tts-c2-synthesis.ts` **179 PASS ×2**（新增 R1-A…R1-J：atomic success
  request fan-out（多 subscriber 同一 claim → 全部 succeeded + 同一 artifact + 全库无
  claim-succeeded-request-active）/ success rollback if exact reread fails / expired validator 不能
  usable-finalize / candidate-hash mismatch 不能 finalize / renewal 仅 current owner / takeover 刷新
  validation_started_at / takeover attempt 2 → dispatch → worker claim（claim/job/attempt_number/
  command.worker_attempt 全 = 2）/ takeover attempt 2 → prestart terminal（command.worker_attempt=2、
  job.attempt=0）/ request replay（行数与 claim 数不变）/ request conflict（fingerprint 变、unit 变）。
  回归：tts-c1b3（267）、tts-c1b2（138）、tts-c1b1（34）、tts-c1c1（53）、voice-library-schema（34）
  全 PASS；typecheck + build PASS。
- **未执行**：m7 merge、production 部署/迁移、POST enable、真实 synthesis；TTS-C.3+ 未启动。

### TTS-C.2 Phase 1 R2 final exact-reread closure（2026-08-07；work branch `work/tts-c2-phase1`；**R2 Final Blocker-Specific Review = PASS（P0=0，P1=0）→ FAST-FORWARD INTEGRATED TO m7 → PRODUCTION DEPLOYMENT = PASS（部署证据 §15.18；PENDING DEPLOYMENT EVIDENCE REVIEW；NOT FROZEN）**）

- **基线**：R1 SHA = `fdd4fac5e088dd24a599f9444b8b57690ec04e59`；R1 Blocker-Specific Review = FAIL
  （P0=0，P1=1：success finalization exact reread closure）。已 PASS 区域（validation fencing /
  attempt propagation / requestId replay / atomic fan-out）未改动。
- **Source artifact content hash 修正**（`synthesis-execution.ts` `sourceArtifactContentHashSync`）：
  弃用行元数据 hash（id/project_id/kind/created_at）；按 exact artifact ID + kind + project 重读
  `content_json` → `sha256(content_json)`（TTS-B canonical 约定，bare hex）。三个 exact source
  （narration_plan_v2 / project_voice_assignment / narration_performance_plan）逐项与待写
  provenance 比对；content_json 缺失 → REQUEST_STATE_INCONSISTENT。
- **Persisted attempt 为 byte/media evidence 权威**：finalize 将 caller 直传 attemptEvidence 与
  persisted file_durable attempt 行逐项比较（final_relative_path / audio_sha256 / output_size /
  codec / sample_rate / channels / ffprobe_duration_ms / provider_request_id）；任一不一致 →
  REQUEST_STATE_INCONSISTENT 整事务回滚。
- **Media metadata exact probe**：最终 durable output 文件 ffprobe 实测（codec / sample_rate /
  channels / duration）与 persisted attempt 逐项一致（frozen §8.2 3a）；`probeAudio` 增加可选
  demuxer 参数（finalize 按 sentence-audio wav 契约传 `'wav'`，规避新版静态 ffmpeg 对正弦数据
  的 mpegts 误判 score 100 > wav 99——默认 auto 时既有调用语义不变，1A 冻结行为未改）。
- **file_durable → succeeded 不再写 byte evidence**：terminal UPDATE 只推进
  `execution_phase='succeeded' / finished_at`（frozen §8.2 step 2）；evidence 保持 file_durable
  冻结原值（R2-J 以测试 trigger 实证：SET 中含任一 byte evidence 列即 ABORT）。
- **Fingerprint 语义重算**（`synthesis-payload.ts` 新增两个纯 helper：
  `computeExactSourceFingerprint`（project/unit + 三 source artifact ID+content hash + voice
  revision pair，固定键序 canonical JSON → sha256:）、`computeFinalTtsInputFingerprint`
  （unit + spokenText + voice revision pair + reference SHA + synthesisPayloadFingerprint））：
  finalize 按 frozen §8.2 3e 重算 exact_source / synthesis_payload（既有）/ final_tts_input /
  generation_variant（Phase-1 canonical = 'default'）并与 request/claim/job persisted 逐项一致；
  测试内 test-only 指纹公式（`exact-${sha256hex(unit)}`、`sha256hex(unit + payloadFp)`）全部移除，
  一律走 production 纯 helper。
- **Voice revision exact 完整化**（`resolveVoiceRevisionExactSync`）：exact pair / provider /
  `adapter_compatibility_key`（frozen `indextts2-adapter-registry@1`）/ canonical SHA 格式 /
  canonical path 精确形状（`voice-library/<pid>/<rid>/reference.wav`）/ containment + parent
  无 symlink / canonical 文件 SHA——与 `validateVoiceProfileRevisionExact` 同一契约的同步子集。
- **测试**：`scripts/test-tts-c2-synthesis.ts` **225 PASS ×8 独立运行**（新增 R2-A…R2-J：content_json
  hash exact / caller content hash 不一致回滚 / 实际 media metadata == persisted PASS /
  media metadata 不一致回滚 / caller evidence ≠ persisted 回滚 / exact_source 语义重算不一致回滚 /
  final_tts_input 语义重算不一致回滚 / generation_variant 非 default 回滚 / voice revision pair
  不一致回滚 / succeeded 不写 byte evidence（trigger 实证）。修复 R1-F validation_started_at
  同毫秒竞态（ISO 毫秒语义下同毫秒无法区分刷新，测试加 5ms 间隔）。回归：tts-c1b3（267）、
  tts-c1b2（138）、tts-c1b1（34）、tts-c1c1（53）、voice-library-schema（34）、
  materialization-worker（20）全 PASS；typecheck + build PASS。
- **未执行（实现期）**：m7 merge、production 部署/迁移、POST enable、真实 synthesis；TTS-C.3+ 未启动。
  → 集成与部署状态见 §15.18（fast-forward 集成保持 reviewed SHA 不变；57-suite gate 已在
  `8e92ba58a28900509e542394858200b1731d7afd` 执行 `QUALITY_GATE_RESULT=PASS`，部署轮不重复运行）。

### 15.18 TTS-C.2 Phase 1 + TTS-C.1C.2 部署证据（8e92ba5，2026-08-08；**PRODUCTION DEPLOYMENT = PASS；PENDING DEPLOYMENT EVIDENCE REVIEW；NOT FROZEN**）

> 详情：`docs/evidence/tts-c2/phase1-deployment.md`（同 commit 独立 evidence 文档）。

- **授权与集成**：R2 Final Blocker-Specific Review = PASS（P0=0，P1=0；ALL IDENTIFIED PHASE-1
  BLOCKERS CLOSED；reviewed SHA `8e92ba58a28900509e542394858200b1731d7afd`）→ m7 fast-forward
  集成（merge-base = `8094885ebc2d81a2bdb57b7fb816bfa2bb683852`，ahead=10/behind=0，ff-only 成功；
  `origin/m7 before = 8094885…` → `after = 8e92ba5…`；reviewed SHA 不变）。集成轮不重复跑
  57-suite gate / 225 assertions ×8 / 全量 typecheck（同一 exact reviewed SHA 已
  `QUALITY_GATE_RESULT=PASS` / `QUALITY_GATE_SHA=8e92ba5…` / `QUALITY_GATE_TOTAL_SUITES=57`；
  仅 `git diff --check` identity proof，无新 SHA 无新代码）。
- **部署链**：previous runtime `cd367f6c3b24b3fe3a3b450e188be7c2cd0eb8d3` →
  deployed `8e92ba58a28900509e542394858200b1731d7afd`（三容器 exact tag；web/worker image
  `150502bbcb43`、adapter image `a70eb64f7951`；restart 全部 0）；host checkout detached
  `8e92ba5…` tracked clean；唯一 env 变更 = `ZHIYING_RELEASE_TAG`（`cd367f6…` → `8e92ba5…`，
  其余行逐行 diff 空；env SHA `74fd75c1…` → `92c9ffea…`）。
- **备份**：`/vol1/1000/backups/zhiying/tts-c2-phase1-20260808T142118Z-8e92ba5`（manifest 入库；
  quiesce 后一致复制 DB SHA `9201fd6ae012e7e0533e5d191aa2774e60b5ed05e7c3fc231dd6986ec5341ec3`
  == 源 DB SHA；WAL checkpoint 无残留；compose×3 / env.backup(600) / voice-registry.json /
  container inspect 全备份）。
- **C.2 migration（代码入口应用，无手工 SQL）**：web 首启（新 runtime）`getDb()` 自动
  `applyTtsC1aMigration → applyTtsC2Migration`（BEGIN IMMEDIATE 幂等）→ 实证 6 新表
  （tts_audio_requests / tts_synthesis_claims / tts_claim_generation_dispatches /
  tts_job_execution_transitions / tts_generation_attempts / sentence_audio_artifacts）、
  tts_jobs 10 additive 列（26 → 36 列）、4 index、65 trigger（frozen 集合精确计数）。migration
  写入 WAL（主 DB 文件 SHA 不变仍 `9201fd6a…`——备份即 pre-migration 精确快照，可作 rollback）。
- **Legacy 兼容**：tts_jobs before=351 → after=351；`claim_id IS NULL` = 351/351；
  claim_id NOT NULL = 0；历史行 9 个 C.2 identity/result/head 字段全部 NULL（0 行非空）。
- **Zero new runtime data**：6 新表全 0 行；publications=0；activations=0；POST remains disabled
  （503 MATERIALIZATION_NOT_ENABLED；env flag 未设）；无真实 synthesis / 无 backfill / 无 C.3+。
- **Build**：既有机制复用（`scripts/production-build-network.sh` start → RUNNING →
  `docker build --network=host --add-host remotion.media:127.0.0.1 --build-arg
  APT_MIRROR=mirrors.aliyun.com --build-arg NPM_REGISTRY=https://registry.npmmirror.com
  -t zhiying:8e92ba5…` BUILD_EXIT=0 → image `150502bbcb43` 2.56GB → tunnel stop STOPPED）；
  adapter 独立上下文 `docker build -t zhiying-indextts2-adapter:8e92ba5…
  services/indextts2-api-adapter`（全层 CACHED；server.py in-image SHA == host SHA；services/
  cd367f6..8e92ba5 diff 空 = 内容一致）。未发明新代理/新 downloader/新 patch。
- **Smoke（production）**：web `GET /api/projects` 200 / `GET /` 200；adapter `/health` 200
  ready=true；`/registry-status` loadedRegistrySha256 `1dab4a31…` 与 preflight 逐字节一致
  （未被 reload；1B.3 registry 行为不变）；worker 启动正常 + "tts-c1b3 registry recovery
  controller started"；web/worker/adapter 日志无 migration error / FK error / trigger error /
  DB locked loop / crash loop / 意外 synthesis / 意外 POST；restart 全部 0。
- **证据边界**：production deployment gate 在 Feiniu 现场执行（命令级证据持久化到 backup +
  evidence 文档）；57-suite quality gate 与 225 assertions 为 agentvm 既有执行记录（exact
  reviewed SHA），本轮不重跑；该 SHA 无 GitHub Actions workflow run/commit status——不得写成
  GitHub-hosted independent CI evidence。
- **状态**：**TTS-C.2 Phase 1 + TTS-C.1C.2：R2 Final Blocker-Specific Review = PASS →
  FAST-FORWARD INTEGRATED TO m7 → PRODUCTION DEPLOYMENT = PASS；PENDING DEPLOYMENT EVIDENCE
  REVIEW；NOT FROZEN**。production runtime =
  `8e92ba58a28900509e542394858200b1731d7afd`；production POST remains disabled；TTS-C.2
  Phase 2（Worker/Scheduler wiring）not started；TTS-C.3+ not authorized。
