...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T04:26Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R8 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1A not started**） |
...
### 11.4 TTS-C.0.R8 docs-only SQLite contract 验证（临时，不入仓库；runtime 阶段纳入 gate）

临时目录 sqlite3 3.45.1（2024-01-30 16:01:20）：**直接从最终设计文档 §2 提取全部可执行 SQL**（12 表：含第 11 表
`voice_registry_publication_activations` 与第 12 表 `tts_claim_generation_dispatches`）重建临时 DB →
`PRAGMA foreign_key_check`（空）→ `PRAGMA integrity_check`（ok）→ happy path 45 项（synthesis 全链含
**单条 atomic dispatch**（CJ-10）/ reuse fan-in / materialization 全链 / **PA-02 materialization 与 PA-03 legacy
cutover 单条 atomic activation command** / registry_rebuild / **crash-retry 闭环**（failed publication A →
...
- **R8 新增矩阵 65 项**：LR-02/02b（rollback 时 publication 非 failed/cancelled → ABORT）、LR-05/05b（pending
  publication ID 非法替换 → ABORT）、PA-01（publication 单独 activation_pending→active → ABORT，无可独立提交的
  active）、PA-04/04b（subject 被抢占整条 statement 回滚）、PA-05（legacy publication 不能激活其他 projection）、
  PA-06/06b（subject 不存在 INSERT ABORT）、PA-07（rebuild subject_id≠'global' ABORT）、PA-08（duplicate
  generation UNIQUE ABORT）、PE-01/01b（lve 无 candidate generation/SHA 列，单一权威）、PE-02/02b/02c
  （indeterminate 期间首补 candidate 证据 ABORT）、PE-03/03b/03c（building-before-candidate indeterminate
  → active ABORT）、CJ-09（generation_pending 无 dispatch ABORT）、CJ-11（dispatch 内 job 失败整条回滚）、
  CJ-12（第二次 dispatch UNIQUE ABORT）、CJ-13/13b（generated 终态仍恰好一个 job）、EN-01…05b（tar/vmr
  waiting 无链接 ABORT / exact 链接 PASS / waiting 清链接 ABORT）、JR-01…05（running/queued/failed 单独写
  result ABORT；status+result 同一更新 PASS；succeeded 后替换 ABORT）；
- **R7 回归 47 项（适配 R8 dispatch/activation 语义）**：RP（publication journal 负向）、CJ-01…07（无环
  claim/job；CJ-01=dispatch command required、CJ-05=result status invariant violated）、SL-01…08b、VI-01…04b；
- **R6 回归 137 项（适配 R8 schema）**：IS-01…20、SM1-9、DEL1-12b（12 表 DELETE 禁 + activation command
  immutable）、PC1-7、CHK1-5/8/9、PAIR1-3、UNIQ1-4、INIT1-7（初始状态直插全拒）。

每个反例的预期错误文本或 changes=0 已逐一断言（同表多 trigger 按创建逆序触发；BEFORE INSERT trigger 先于 FK/CHECK
enforcement——冻结消息以实证为准）。临时 SQL/DB 未入仓库。
