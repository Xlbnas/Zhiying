...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-03T12:00Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R2 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1A not started**） |
...
- **TTS-C.0.R2 architecture closure completed（pending independent Review PASS；runtime implementation not started；TTS-C.1A not started；只读审计，零代码/零部署）**：关闭独立 Review FAIL 发现——① **不同 requestId 的 synthesis single-flight**：强制 partial unique `uq_tts_jobs_active_synthesis`（project_id, unit_id, final_tts_input_fingerprint, generation_variant_id WHERE status IN ('queued','running','indeterminate')）；indeterminate 继续占 claim；不同 requestId 同 synthesis key 链同一 active job；**不设 artifact fingerprint UNIQUE**（损坏 replacement/显式重复生成/provider 非 byte-deterministic 多 candidate 合法共存）；两阶段事务算法（BEGIN IMMEDIATE 同步 DB 裁决 → 事务外 exact file validator → 重入 BEGIN IMMEDIATE 复用/repair）；② **phase-aware crash recovery**（7 阶段矩阵：queued/claimed_pre_provider 可 requeue、provider_in_flight→indeterminate 禁自动重调、response_received/file_durable 本地恢复不重调、db_completed exact reader 验证；**不得仅凭 status='running' 无条件 requeue**）；indeterminate resolution 三选一 + usage/cost 保留；③ **原子成功终局事务**（文件 temp→probe/SHA/size/codec→rename→fsync 先于 DB；单 BEGIN IMMEDIATE 内 artifact/attempt/job/envelope 四者原子，cancel 优先，失败整事务回滚 + durable 文件为 orphan + cleanup 不触发第二次 provider）；④ **attempt journal 修正**（append-one-row-per-provider-call；immutable request identity + mutable lifecycle + 禁 DELETE + DB trigger 限状态倒退）；⑤ **fingerprint vs generation variant 修正**（materialization transport 不入 fingerprint；generationVariantId = 候选生成身份，opaque 不伪称 provider payload；同 fingerprint 下显式多 candidate；adapter 无 variation 通道时 UI 标 "repeat generation" 不声称参数化 A/B）；⑥ **registry cutover**（1B 前 legacy import 严格验证 + 全等性断言才切换；不得空 registry 替换；无法映射 entry fail-closed 人工裁决；empty registry/LKG health 语义冻结：candidate invalid + LKG → ready/degraded/detail=VOICE_REGISTRY_PUBLISH_FAILURE，无 LKG → ready=false + 503）；⑦ **1A 状态机边界**（requested/file_writing/file_ready_unpublished/registry_pending/published_usable/failed/indeterminate；1A 只到 file_ready_unpublished，API 不声称 adapter ready、TTS dispatch 不可用；`voice_materialization_state` 精确 schema + request envelope 已冻结）；⑧ **OpenMontage 真实代码审计修正**（calesthio @ `4eab34c5`：checkpoint persistence/canonical artifact validation/JSON Schema/stage prerequisite/human gates/pipeline manifests/video_compose 全部真实存在，Reject 依据 = AGPL license/integration mismatch；Open-Montage @ `50279751…` 修正 SHA）。三份 TTS-C 文档全部更新；TTS-B/A 冻结语义未动。
