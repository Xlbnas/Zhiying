...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T13:10Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R12 architecture revision completed，pending independent Review；TTS-C runtime implementation not started；TTS-C.1A / 1B / 1C not started**） |
...
### 11.4 TTS-C.0.R12 docs-only SQLite contract 验证（runner 与原始输出入库 `docs/evidence/tts-c-r12/`；runtime 阶段纳入 gate）
...
（110 triggers / 7 unique indexes + 2 表级 UNIQUE(job_id|claim_id,command_seq) / 10 ALTER ADD COLUMN）
→ **真实事务能力**（PyEngine 同一 connection BEGIN IMMEDIATE→逐条→COMMIT/ROLLBACK；CLI 同一进程
脚本 BEGIN IMMEDIATE;…;COMMIT; -bail，失败后新连接验证回滚）→ 逐测试执行：**130 项 mutation
...
逐 test PASS/FAIL、总数；任一 FAIL 非零 exit）：

- **R12 新增矩阵 20 项（HR-01…20，historical command replay seal，全部真实历史值）**：历史
  token/lease/heartbeat/attempt 重放全 ABORT（HR-01…05）、terminal owner/证据复活全拒
  （HR-06/07/08）、head 回退/跳号/单侧推进/复用全拒（HR-09…13）、chain 断裂与 seq 冲突
  （HR-14/15）、有效 renewal/takeover/state_transition 链推进（HR-16/17/18）、第二侧故障整事务
  回滚（HR-19）、历史多行全量重放（HR-20）；
- **R11 全矩阵重跑 110 项**：JS-01…35（五类 command + per-column fence）、LC-01…12、TF-01…08、
  IE 全组、VI 全组、PA/CJ/JR/EN/SM/ET/GN/LR/PE/RP；
...
- **R11/R10/R9 口径**：R11 的 110 项被覆盖；R10 的 91 项、R9 的 360/23-29 不再引用。
- 每个反例的预期错误文本或 `changes=0` 已逐一断言（同表多 trigger 按创建逆序触发：terminal shape
  trigger 可先于 per-column fence 报 `terminal owner shape violated`；UNIQUE(job_id,command_seq) 与
  UNIQUE(claim_id,command_seq) 冲突时报告其一——均为合法拒绝，runner 用消息集合断言）。
  未提交临时 DB；未生成音频。checked-in results 记录生成时 base HEAD（`git HEAD` 字段），
  **final HEAD 的权威绑定由 GitHub CI artifact（final checkout 重新生成）提供**。

### 11.4.1 CI TTS-C Contract Gate（R12：sqlite3 fail-closed pin）

`.github/workflows/m7-quality-gate.yml` 的 `TTS-C Contract Gate` job 指向
`docs/evidence/tts-c-r12/`：ubuntu-24.04 + setup-python 3.12；**sqlite3 CLI 版本 fail-closed
pin**——`ACTUAL="$(sqlite3 --version | awk '{print $1}')"`、`test "$ACTUAL" = "3.45.1"`
（不匹配即 workflow failure，不依赖 `sqlite3 --version` 打印冒充 pin）；Python sqlite3 版本记录
并报告（CLI = authoritative compatibility engine，文档准确写 Python engine = workflow 实际版本）；
