...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T09:51Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R10 architecture revision completed，pending independent Review；TTS-C runtime implementation not started；TTS-C.1A / 1B / 1C not started**） |
...
- 下一步：TTS-C.1A（voice materialization durable requests/files/DB projection，schema 按 R10 contract）→ TTS-C.1B（global registry publisher + adapter hot reload，mapping_mode/subject_mode 双路径就位）→ TTS-C.1C（capability snapshot/compiler）→ TTS-C.2（request envelope + durable job + attempt journal + immutable sentence audio artifact + fingerprints + `tts_job_execution_transitions` 双 command 语义 atomic coupling）→ TTS-C.3（preview/override/variant/A-B/selection）→ TTS-C.4（selection manifest + immutable master audio + ffprobe）→ TTS-C.5（subtitle timing v2/reconciliation/stale graph/review UI）→ timing-reconciliation@2.0 → storyboard → animatic → final render。TTS-C.1A / 1B / 1C 未开始，须待本 R10 独立 Review PASS。
...
### 11.4 TTS-C.0.R10 docs-only SQLite contract 验证（runner 与原始输出入库 `docs/evidence/tts-c-r10/`；runtime 阶段纳入 gate）
...
第 13 表 `tts_job_execution_transitions`；不维护手写 schema 副本；既有基座表按 §0 基座前提以最小
fixture 提供）→ 双引擎（sqlite3 CLI 3.45.1 + 当前 Python sqlite3）各自重建临时 DB →
`PRAGMA foreign_key_check`（空）→ `PRAGMA integrity_check`（ok）→ 对象计数（96 triggers /
7 unique indexes / 8 ALTER ADD COLUMN）→ 逐测试执行：**91 项 mutation 断言全部按预期，FAIL=0，
两引擎一致**（原始输出 `results-sqlite-3.45.1.txt` / `results-python-sqlite.txt`，含 git HEAD、
design doc sha256、extracted §2 sql sha256、逐 test PASS/FAIL、总数；任一 FAIL 非零 exit）：

- **R10 新增矩阵 30 项**：JS-01…17+11b（worker_claim 首次 ownership establishment / state_transition
  owner fencing / 全生命周期多 transition / 语义防重 / takeover CAS / 原子回滚）18 项；
  LC-01…12（publish_and_cutover 与 cutover_existing 双路径可达 / 竞争裁决三情况 / cardinality 与
  retired 语义 / rollback→retry）12 项；EA-01/03 由 runner 输出自证；
- **R9 矩阵重跑 39 项**：TF-01…08（database-time fencing：过期回填 activated_at/created_at/
  dispatch.created_at 全 ABORT、lease 未过期 exact PASS、takeover 后旧 owner 回填 ABORT、fence `<=`
  含等值 / 过期 `<` 严格 / julianday 截断漂移 ∈ {-1,0}）、IE-01…14+06b+08a/b（indeterminate entry
  evidence seal / shape / exact-attempt resolve / resolution_evidence 必填）、VI-05…08+07b
  （voice revision compat：修改拒绝 / 不一致 ABORT / CAST 格式边界 / legacy 隔离）、
  SM-01…05（subject_mode 联合判定）、ET-01…03（evidence timestamp freeze）、GN-01/02（generation UNIQUE）；
- **R8 回归重跑 20 项**：PA-01/04/06/07（activation command 唯一入口 / replay 拒绝 / subject 校验 /
  registry_rebuild 全链）、CJ-09…13（exact-one atomic dispatch / 回滚 / UNIQUE / 终态恰好一个 job）、
  JR-01/02/03/05（job result row-state invariant / write-once）、EN-01…05（envelope waiting link
  closure）、PE-03（building 来源 indeterminate 不得 active）、LR-01（rollback 清证据 + 旧 evidence 保留）；
- **R7 回归重跑 2 项**：RP-11/12（publication lease fencing renewal）。
...
- **R9 口径作废**：R9 的"360 项 FAIL=0"（枚举与总数矛盾、runner 未入库）与实施报告 23/29 均不再引用；
  其中 JS-03/LC-02 经独立 Review 与本 runner 证实为真实 schema 缺陷（P0-1/P0-3），非 fixture 问题。
- 顺带实证修复 D1：`trg_tts_jobs_immutable` 误将 indeterminate 当终态冻结（JS-10），已收窄为真终态。

每个反例的预期错误文本或 `changes=0` 已逐一断言（同表多 trigger 按创建逆序触发——替换 result/job
链接时 link/identity trigger 可先于 immutable/invariant 报 ABORT，runner 用消息集合断言，均为合法
拒绝）。未提交临时 DB；未生成音频。
