...
- **TTS-C.1B.2 implemented on work branch（pending independent Review；not merged；not deployed；production 不变）**：legacy import + publisher candidate creation（T1 claim / candidate 确定性构建 / T2 file_durable）——见实施计划 `docs/TTS_C_1B_1C_EXECUTION_PLAN.md` §C 步骤 1-3 / §F / §J。真实代码入口：
