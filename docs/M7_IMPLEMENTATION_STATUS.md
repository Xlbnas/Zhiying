...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T15:35Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0 = FROZEN（独立 Review PASS）**；**TTS-C.1A.R1 deployed（pending independent Review PASS；not frozen）**；TTS-C.1B / 1C / C.2 not started） |
...
| productionRuntimeSHA | `71d87b494d6ce08f6753990a3b8aeb9a29ad3530`（TTS-C.1A.R1 部署后容器镜像实际代码 SHA；TTS-C.0 freeze 基线与 CI 证据见 §11.4） |
| productionHostCheckoutSHA | `71d87b494d6ce08f6753990a3b8aeb9a29ad3530`（宿主机 checkout；可因 docs/ops commit 高于 runtime） |
| 当前分支 | `m7` |
