...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-03T13:00Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R3 architecture closure completed，pending independent Review PASS；TTS-C runtime implementation not started；TTS-C.1A not started**） |
...
- **TTS-C.0.R3 architecture closure completed（pending independent Review PASS；runtime implementation not started；TTS-C.1A not started；只读审计，零代码/零部署）**：关闭独立 Review FAIL 发现——① **unschedulable reuse reservation**（新增 `tts_synthesis_claims`：active synthesis identity 唯一 reservation，partial unique `WHERE status IN ('validating_reuse','generation_pending','running','indeterminate')`；`validating_reuse` Scheduler 永不 claim——**修复"校验前创建 queued job 被 claim"竞态**；artifact usable 不建 job，unusable 才在 claim 保护下转 queued；三阶段算法 + Scheduler 竞态/usable 零新 job/damaged 恰好一个 queued/并发两 requestId 单 claim 测试）；② **共享 request fan-in**（`tts_audio_requests` many-to-one → claim/job；`tts_jobs` 不设单数 request owner FK（仅 `originating_request_id`）；成功/失败 fan-out 全部有效 subscriber，任一 request identity mismatch → 整事务回滚 + REQUEST_STATE_INCONSISTENT；per-request cancel 仅 detach 该 envelope，全部取消才置 job `cancel_requested=1`，success 与最后 cancel 原子裁决；A+B 共享/A cancel B 成功/双 cancel/并发测试矩阵）；③ **persisted execution phases**（`tts_generation_attempts.execution_phase`：created/provider_in_flight/response_persisted/file_validated/file_durable/succeeded/transport_failed/validation_failed/indeterminate + recovery temp/final/audio evidence 列；provider 调用前 INSERT attempt 先 COMMIT；recovery 按 phase：provider_in_flight → indeterminate、response_persisted/file_validated/file_durable → 本地恢复不重调；recovery 校验 attempt/job/claim/source/fingerprint/owner）；④ **原子 claim/job/request/artifact 成功终局**（9 步单 BEGIN IMMEDIATE：重读 claim/job/attempt/全部 subscriber → INSERT artifact → attempt/job/claim/requests 逐一 succeeded → COMMIT；失败整事务回滚 + recoverable orphan + 不调用 provider；无部分成功；cancel 优先）；⑤ **materialization 三层拆分**（`voice_materialization_requests` envelope + `voice_materialization_jobs` mutable execution + `voice_materializations` canonical projection `UNIQUE(profile, revision)`；1A 止于 `file_ready_unpublished`，目标路径固定 voice-root-relative；1A durability 流程冻结）；⑥ **legacy registry shadow**（`legacy_adapter_voice_entries` 独立表，不伪造 TTS-A 数据；mapping_status unmapped/mapped/retired；publisher source union = non-retired legacy + published/registry_pending materializations；canonical key 唯一冲突 fail-closed；cutover 前后 legacy 集合与 SHA 完全保持）；⑦ **adapter activation acknowledgement**（registryGeneration + publisherSchemaVersion + /health activeRegistrySha256/candidateRegistrySha256/degraded；publish → registry_pending → poll → active SHA == published → published_usable；LKG 保持时不得标 usable）；⑧ **自包含最终 schema**（设计文档完整写出 9 张表：tts_audio_requests/tts_synthesis_claims/tts_jobs TTS-C 新列/tts_generation_attempts/sentence_audio_artifacts/voice_materialization_requests/voice_materialization_jobs/voice_materializations/legacy_adapter_voice_entries——PK/FK/UNIQUE/partial UNIQUE/CHECK/triggers/状态机/exact reader/authoritative source/file-DB durability/API path redaction/migration compat；实施者不得跨历史 commit 拼接 schema）；⑨ **OSS 矩阵 license 措辞去绝对化**。三份 TTS-C 文档全部更新；TTS-B/A 冻结语义未动。
