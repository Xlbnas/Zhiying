...
| 字段 | 值 |
|---|---|
| statusUpdatedAt | 2026-08-04T12:00Z（M7.3B FROZEN；**TTS-A FROZEN**；**TTS-B FROZEN（独立 Review PASS）**；**TTS-C.0.R11 architecture revision completed，pending independent Review；TTS-C runtime implementation not started；TTS-C.1A / 1B / 1C not started**） |
...
### 11.4 TTS-C.0.R11 docs-only SQLite contract 验证（runner 与原始输出入库 `docs/evidence/tts-c-r11/`；runtime 阶段纳入 gate）
...
第 13 表 `tts_job_execution_transitions`（R11 五类 command）；不维护手写 schema 副本；既有基座表
按 §0 基座前提以最小 fixture 提供）→ 双引擎（sqlite3 CLI 3.45.1 + 当前 Python sqlite3）各自重建
临时 DB → `PRAGMA foreign_key_check`（空）→ `PRAGMA integrity_check`（ok）→ 对象计数
（107 triggers / 11 unique indexes / 8 ALTER ADD COLUMN）→ **真实事务能力**（PyEngine 同一
connection BEGIN IMMEDIATE→逐条→COMMIT/ROLLBACK；CLI 同一进程脚本 BEGIN IMMEDIATE;…;COMMIT;
-bail，失败后新连接验证回滚）→ 逐测试执行：**110 项 mutation 断言全部按预期，FAIL=0，两引擎逐
test 一致**（原始输出 `results-sqlite-3.45.1.txt` / `results-python-sqlite.txt`，含 git HEAD、
design doc sha256、extracted §2 sql sha256、逐 test PASS/FAIL、总数；任一 FAIL 非零 exit）：

- **R11 新增矩阵 25 项（JS-18…35）**：直接修改 fence（JS-18/19/20/21/24：running claim 单改
  owner / running job 单改 claimed_by/attempt / 单改 heartbeat / terminal 后 owner 复活全拒）、
  lease_renewal 原子（JS-22/23/31/32）、execution_takeover 原子（JS-24b/25/26b：真实时序过期
  接管 attempt+1、旧 owner 全拒）、prestart_terminal 可达（JS-27/28/29/35）、shape CHECK
  （JS-30）、真实 BEGIN IMMEDIATE 事务（JS-33/34：成功提交 + 跨语句回滚）、R10 两条 UPDATE CAS
  作废实证（JS-17）；
- **R10 全矩阵重跑 85 项**：JS-01…16（worker_claim/state_transition 生命周期）、LC-01…12
  （双路径 cutover / 竞争裁决 / cardinality）、TF-01…08（database-time fencing）、IE 全组
  （indeterminate seal/resolve）、VI 全组（voice revision compat）、PA/CJ/JR/EN/SM/ET/GN/LR/PE/RP
  （activation/dispatch 原子性、result invariant、envelope closure、generation UNIQUE 等）；
...
- **R10/R9 口径**：R10 的 91 项已由 R11 全矩阵覆盖；R9 的 360/23-29 不再引用。
- 每个反例的预期错误文本或 `changes=0` 已逐一断言（同表多 trigger 按创建逆序触发：claim 侧
  lease fence 可先于 status fence 报 `lease requires execution command`；runner 用消息集合断言，
  均为合法拒绝）。未提交临时 DB；未生成音频。

### 11.4.1 CI TTS-C Contract Gate（R11 P1-B 关闭）

`.github/workflows/m7-quality-gate.yml` 新增 `TTS-C Contract Gate` job：ubuntu-24.04
（自带 sqlite3 3.45.1）+ setup-python 3.12；`--engine all` 在 **final HEAD** 重新生成双引擎结果
（不依赖 checked-in result 伪装执行）；`verify_engines.py` 断言两引擎 test ID 与
TOTAL/PASS/FAIL/SKIP 完全一致、design/SQL SHA 与 current checkout 一致；FAIL>0 → workflow
failure；结果上传 artifact `tts-c-contract-gate-<sha>`；summary 写 contract total、两引擎 SQLite
版本、design SHA、SQL SHA。**与原有 27 个 runtime suite 分别报告，不混称。**
