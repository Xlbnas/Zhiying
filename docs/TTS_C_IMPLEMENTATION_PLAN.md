...
- 新表：`tts_audio_requests`（**R8-H**：initializing→waiting 必须 exact claim 链接，`waiting requires claim link`）、
  ...
  **`tts_job_execution_transitions`（R13 ⑥ 第 13 表）**（append-only execution coupling command：
  ...
  command 行 previous_command_id + command_seq（UNIQUE(job_id|claim_id, command_seq)）；
  **direct mutation fence 绑定 NEW head**：任何受保护字段变化必须推进 head 到唯一 command e
  且字段与 e 逐项一致——历史 command 值不能授权直接 UPDATE；head 不可回退/跳号/单侧推进/
  重复消费；trg_tjs_terminal_shape 强制 terminal claimed_* NULL；全部 TTS-C 状态与 owner 变更
  必须精确匹配 command 行）、
