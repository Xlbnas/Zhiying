# TTS-C.0.R13 reproducible contract evidence

本目录是 TTS-C.0.R13 的可复跑证据：**只从** `docs/TTS_C_INCREMENTAL_NARRATION_DESIGN.md` §2
...
```bash
python3 docs/evidence/tts-c-r13/verify_engines.py \
...
| `test_hr.py` | HR-01…20：historical command replay seal（applied-command chain，真实历史值） |
| `test_ce.py` | CE-01…20：claim terminal evidence seal + post-validation evidence seal |
...
- 本轮实际执行 150 项，PASS=150，FAIL=0，SKIP=0（两引擎逐 test 一致）；PASS+FAIL+SKIP=总数。
- R13 核心：claim terminal evidence seal（trg_tsc_terminal_immutable）+ post-validation evidence
  seal（trg_tsc_post_validation_evidence_seal）——CE-01…20 实证。
- R12 核心：applied-command chain（execution head）——历史 command 值不能授权任何直接 UPDATE
  （HR-01…20 实证）；当前 head 行完整重放 = 零变化 no-op（改值变体被拒）。
...
- CI（`.github/workflows/m7-quality-gate.yml` 的 `TTS-C Contract Gate` job）在 final HEAD 重新
  生成双引擎结果，不依赖 checked-in result 伪装执行；sqlite3 CLI 版本 fail-closed pin 3.45.1。
- checked-in results 的 `git HEAD` 字段记录生成时 base HEAD；final HEAD 权威绑定由 CI artifact 提供。
