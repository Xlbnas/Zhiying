...
class CliEngine:
    ...
    def _run(self, sql, capture=False):
        # 每个 CLI 子进程独立 session：内存 journal + 关同步（临时测试 DB），
        # 消除共享存储 fsync 卡顿；PRAGMA 输出用 .output /dev/null 抑制避免污染查询结果
        script = (".nullvalue %s\n.output /dev/null\nPRAGMA journal_mode=MEMORY;\n"
                  "PRAGMA synchronous=OFF;\n.output stdout\nPRAGMA foreign_keys=ON;\n%s"
                  % (self.NULL_SENTINEL, sql))
    ...
    def tx_run(self, statements):
        """同一 sqlite3 进程内执行 BEGIN IMMEDIATE; stmt1; ...; COMMIT;。
        任一失败（-bail 非零）→ 未提交，连接关闭时 SQLite 自动回滚；
        调用方随后用新连接验证全部回滚。"""
        stmts = ";\n".join(s.rstrip().rstrip(";") for s in statements) + ";"
        script = (".nullvalue %s\n.output /dev/null\nPRAGMA journal_mode=MEMORY;\n"
                  "PRAGMA synchronous=OFF;\n.output stdout\nPRAGMA foreign_keys=ON;\n"
                  "BEGIN IMMEDIATE;\n%s\nCOMMIT;\n" % (self.NULL_SENTINEL, stmts))
        p = subprocess.run([self.bin, "-batch", "-bail", "-list", self.dbfile],
                           input=script, capture_output=True, text=True)
        if p.returncode != 0:
            msg = (p.stderr or p.stdout).strip().replace("\n", " | ")
...
class Harness:
    ...
    def worker_claim(self, tid, jid="J1", cid="C1", token="w1", lease_ms=None, attempt=1,
        ...
        """R12：第一条 command——previous_command_id IS NULL、command_seq=1（target head NULL/0）。"""
        if lease_ms is None:
            lease_ms = now_ms() + 600000
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            ...
            VALUES (%s,%s,%s,%s,NULL,1,'worker_claim','generation_pending','running','queued','running',
                    %s,%d,%d,%s,%s,%s,%s)"""
    ...
    def state_transition(self, tid, jid="J1", cid="C1", frm="running", to="succeeded",
                         token="w1", attempt=1, result=None,
                         error_code=None, error_message=None, reason=None,
                         activated=None, created=None):
        """R11：state_transition 不携带 lease（worker_lease NULL）；failed 必带 error_code；
        cancelled/indeterminate 必带 reason 或 error_code（表 CHECK 强制）。"""
        activated = activated or iso(0)
        created = created or iso(0)
        ...
        rn = "NULL" if reason is None else q(reason)
        prev, seq = self._chain_prev_seq(cid)
        self.exec("""INSERT INTO tts_job_execution_transitions
            (id,transition_request_id,job_id,claim_id,previous_command_id,command_seq,command_kind,
             from_claim_status,to_claim_status,from_job_status,to_job_status,
             worker_owner_token,worker_attempt,
             result_artifact_id,error_code,error_message,reason,activated_at,created_at)
            VALUES (%s,%s,%s,%s,%s,%d,'state_transition',%s,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), prev, seq, q(frm), q(to), q(frm), q(to),
                     q(token), attempt, res, ec, em, rn, q(activated), q(created)))
    ...
    def tx(self, statements):
        """真实同事务执行（双引擎）：BEGIN IMMEDIATE → 逐条 → COMMIT；
        任一失败整事务回滚并抛 SqlError。用于跨语句原子性/回滚测试。"""
        if isinstance(self.ex, PyEngine):
            try:
                self.ex.tx_begin()
                for s in statements:
                    self.ex.tx_exec(s)
            ...
            self.ex.tx_run(statements)

    def lease_renewal(self, tid, jid="J1", cid="C1", token="w1", attempt=1,
                      new_lease=None, heartbeat=None, activated=None, created=None, frm="running"):
        """R12：同态续租（running→running 或 indeterminate→indeterminate，frm 参数指定）。"""
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            ...
            VALUES (%s,%s,%s,%s,%s,%d,'lease_renewal',%s,%s,%s,%s,
                    %s,%d,%d,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), prev, seq, q(frm), q(frm), q(frm), q(frm),
                     q(token), new_lease, attempt,
                     q(heartbeat), q(activated), q(created)))

    def execution_takeover(self, tid, jid="J1", cid="C1", new_token="w2", attempt=2,
                           new_lease=None, claimed=None, heartbeat=None, activated=None, created=None,
                           frm="running"):
        """R12：同态接管（running→running 或 indeterminate→indeterminate，frm 参数指定）。"""
        if new_lease is None:
            new_lease = now_ms() + 900000
        claimed = claimed or iso(0)
        heartbeat = heartbeat or iso(0)
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            (id,transition_request_id,job_id,claim_id,previous_command_id,command_seq,command_kind,
             ...
             worker_owner_token,worker_lease_expires_at_epoch_ms,worker_attempt,
             claimed_at,heartbeat_at,activated_at,created_at)
            VALUES (%s,%s,%s,%s,%s,%d,'execution_takeover',%s,%s,%s,%s,
                    %s,%d,%d,%s,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), prev, seq, q(frm), q(frm), q(frm), q(frm),
                     q(new_token), new_lease, attempt,
                     q(claimed), q(heartbeat), q(activated), q(created)))

    def prestart_terminal(self, tid, jid="J1", cid="C1", to_status="failed", attempt=1,
                          error_code=None, error_message=None, reason=None,
                          activated=None, created=None):
        activated = activated or iso(0)
        created = created or iso(0)
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            (id,transition_request_id,job_id,claim_id,previous_command_id,command_seq,command_kind,
             from_claim_status,to_claim_status,from_job_status,to_job_status,
             worker_attempt,error_code,error_message,reason,activated_at,created_at)
            VALUES (%s,%s,%s,%s,NULL,1,'prestart_terminal','generation_pending',%s,'queued',%s,
                    %d,%s,%s,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), q(to_status), q(to_status),
                     attempt, ec, em, rn, q(activated), q(created)))

    def _chain_prev_seq(self, cid):
        """R12：从 claim 读当前 execution head，返回 (previous_command_id, command_seq)。"""
        row = self.query("SELECT last_execution_command_id, execution_command_seq"
                         " FROM tts_synthesis_claims WHERE id=%s" % q(cid))
        prev = row[0][0]
        seq = int(row[0][1]) + 1
        if prev is None:
            prev = "NULL"
...
def load_tests(evidence_dir):
    modules = ["test_js", "test_lc", "test_tf", "test_ie", "test_vi", test_regress_name(), "test_hr", "test_ce"]
    tests = []
    for name in modules:
...
def run_engine(kind, args, info, contract_sql, evidence_dir, results_dir):
    ...
    with tempfile.TemporaryDirectory(prefix="ttsc-r13-") as tmpdir:
        # schema 验证（一次性）
