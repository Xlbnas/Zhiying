# TTS-C.0.R12 reproducible contract evidence

本目录是 TTS-C.0.R12 的可复跑证据：**只从** `docs/TTS_C_INCREMENTAL_NARRATION_DESIGN.md` §2
...
```bash
python3 docs/evidence/tts-c-r12/contract_runner.py \
...
```bash
python3 docs/evidence/tts-c-r12/verify_engines.py \
...
- 本轮实际执行 130 项，PASS=130，FAIL=0，SKIP=0（两引擎逐 test 一致）；PASS+FAIL+SKIP=总数。
