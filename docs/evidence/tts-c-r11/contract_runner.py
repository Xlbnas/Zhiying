...
class Harness:
    ...
    def state_transition(self, tid, jid="J1", cid="C1", frm="running", to="succeeded",
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            ...
            VALUES (%s,%s,%s,%s,'state_transition',%s,%s,%s,%s,%s,%d,%s,%s,%s,%s,%s,%s)"""
    ...
    def lease_renewal(self, tid, jid="J1", cid="C1", token="w1", attempt=1,
                      new_lease=None, heartbeat=None, activated=None, created=None):
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            ...
            VALUES (%s,%s,%s,%s,'lease_renewal','running','running','running','running',
                    %s,%d,%d,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), q(token), new_lease, attempt,
    ...
    def execution_takeover(self, tid, jid="J1", cid="C1", new_token="w2", attempt=2,
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            (id,transition_request_id,job_id,claim_id,command_kind,
             ...
             worker_owner_token,worker_lease_expires_at_epoch_ms,worker_attempt,
             claimed_at,heartbeat_at,activated_at,created_at)
            VALUES (%s,%s,%s,%s,'execution_takeover','running','running','running','running',
                    %s,%d,%d,%s,%s,%s,%s)"""
                  % (q(tid), q("req-" + tid), q(jid), q(cid), q(new_token), new_lease, attempt,
                     q(claimed), q(heartbeat), q(activated), q(created)))
    ...
    def prestart_terminal(self, tid, jid="J1", cid="C1", to_status="failed", attempt=1,
        ...
        self.exec("""INSERT INTO tts_job_execution_transitions
            ...
            VALUES (%s,%s,%s,%s,'prestart_terminal','generation_pending',%s,'queued',%s,
                    %d,%s,%s,%s,%s,%s)"""
