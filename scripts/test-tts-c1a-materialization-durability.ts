...
(async () => {
  ...
  if (realProj) {
    ...
    const vr = await validateExistingProjection(realProj as unknown as never, null, {jobId: '', validationOwnerToken: '', validationAttempt: 0, validationLeaseExpiresAtEpochMs: 0, candidateMaterializationId: ((realProj as unknown) as {id: string}).id, candidateMaterializationMetadataHash: null}, 'unknown');
